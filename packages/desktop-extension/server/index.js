...
// packages/engine/src/retrieval/types.ts
var DEFAULT_FUSION_WEIGHTS;
...
// packages/engine/src/retrieval/compound-search.ts
async function compoundSearchSignal(storage, embedding, query, tenantId, scope, scopeId, limit) {
  const queryEmbedding = await embedding.embed(query);
  const keywordQuery = query.toLowerCase().replace(/[^a-z0-9\s]/g, " ").split(/\s+/).filter((w) => w.length >= 2 && !QUERY_STOP_WORDS.has(w)).join(" | ");
  const results = await storage.compoundSearch({
    ...
    limit,
    minSimilarity: 0.25
    // low threshold — let fusion scoring handle ranking, not hard cutoffs
  });
  ...
  for (const r2 of results) {
    if (r2.source === "vector") {
      ...
      keywordCandidates.push({
        fact: r2.fact,
        vectorScore: 0,
        keywordScore: r2.relevanceScore,
        graphScore: 0,
        recencyScore: 0,
        salienceScore: 0,
        temporalScore: 0,
  ...
  if (keywordCandidates.length > 0) {
    const maxRank = Math.max(...keywordCandidates.map((c2) => c2.keywordScore));
    if (maxRank > 0) {
      for (const c2 of keywordCandidates) {
        c2.keywordScore = c2.keywordScore / maxRank;
      }
    }
  }
...
var init_compound_search = __esm({
  "packages/engine/src/retrieval/compound-search.ts"() {
    "use strict";
...
// packages/engine/src/retrieval/graph-traversal.ts
function tokenizeQuery(query) {
  return query.split(/[\s.]+/).map((t3) => t3.replace(/[^\w-]/g, "")).filter((t3) => t3.length >= MIN_TOKEN_LENGTH).map((t3) => t3.toLowerCase());
}
async function graphSearch(storage, embedding, query, tenantId, _scope, _scopeId, limit, config) {
  ...
  if (tokens.length >= 2) {
    for (let i3 = 0; i3 < tokens.length - 1; i3++) {
      candidateNames.push(`${tokens[i3]} ${tokens[i3 + 1]}`);
      candidateNames.push(`${tokens[i3]}.${tokens[i3 + 1]}`);
    }
  }
  ...
  try {
    const { data } = await storage.client.from("entities").select("id, canonical_name").eq("tenant_id", tenantId).in("canonical_name", candidateNames);
    if (data) {
    ...
    for (const token of tokens) {
      for (const entityType of ENTITY_TYPES) {
        const entity2 = await storage.findEntityByCanonicalName(tenantId, token, entityType);
        if (entity2 && !seedEntityIds.includes(entity2.id)) {
          seedEntityIds.push(entity2.id);
        }
      }
    }
  }
  ...
  const entityHopMap = /* @__PURE__ */ new Map();
  for (const entity2 of traversalResult.entities) {
    if (seedEntityIds.includes(entity2.id)) {
      entityHopMap.set(entity2.id, 0);
    }
  }
  ...
  for (const entity2 of traversalResult.entities) {
    if (!entityHopMap.has(entity2.id)) {
      entityHopMap.set(entity2.id, maxDepth);
    }
  }
  const candidateMap = /* @__PURE__ */ new Map();
  const entityIds = traversalResult.entities.map((e2) => e2.id);
  if (entityIds.length > 0) {
    ...
    try {
      ...
      for (const entity2 of traversalResult.entities) {
        const hopDepth = entityHopMap.get(entity2.id) ?? maxDepth;
        const graphScore = 1 / Math.pow(2, hopDepth);
        try {
          const factsResult = await storage.getFactsForEntity(tenantId, entity2.id, { limit: 3 });
          ...
          console.error("[steno-graph] getFactsForEntity failed for entity:", entity2.id, err2 instanceof Error ? err2.message : err2);
        }
      }
    }
  }
  return Array.from(candidateMap.values()).sort((a2, b) => b.graphScore - a2.graphScore).slice(0, limit);
}
var DEFAULT_MAX_DEPTH, MAX_ALLOWED_DEPTH, DEFAULT_MAX_ENTITIES, MIN_TOKEN_LENGTH, ENTITY_TYPES;
var init_graph_traversal = __esm({
  "packages/engine/src/retrieval/graph-traversal.ts"() {
    "use strict";
    DEFAULT_MAX_DEPTH = 2;
    MAX_ALLOWED_DEPTH = 5;
    ...
    MIN_TOKEN_LENGTH = 2;
    ENTITY_TYPES = ["person", "organization", "location", "topic", "concept", "product", "event"];
  }
});

// packages/engine/src/retrieval/vector-search.ts
async function vectorSearch(storage, embedding, query, tenantId, scope, scopeId, limit, asOf) {
  const queryEmbedding = await embedding.embed(query);
...
// packages/engine/src/retrieval/trigger-matcher.ts
async function matchTriggers(storage, embedding, query, tenantId, scope, scopeId) {
  const triggers = await storage.getActiveTriggers(tenantId, scope, scopeId);
  const candidates = [];
  const triggersMatched = [];
  for (const trigger of triggers) {
    const matches3 = await evaluateCondition(trigger.condition, query, {
      storage,
      embedding,
      tenantId,
      scope,
    ...
    if (matches3) {
      ...
      if (trigger.queryTemplate) {
        const templateResults = await vectorSearch(
          storage,
          embedding,
          trigger.queryTemplate,
          tenantId,
          scope,
          scopeId,
          5
        );
        for (const c2 of templateResults) {
          candidates.push({ ...c2, source: "trigger", triggeredBy: trigger.id });
        }
      }
...
async function evaluateCondition(condition, query, context) {
  const lowerQuery = query.toLowerCase();
  if (condition.AND && condition.AND.length > 0) {
    const results = await Promise.all(
      condition.AND.map((sub) => evaluateCondition(sub, query, context))
    );
    return results.every(Boolean);
  }
  if (condition.OR && condition.OR.length > 0) {
    const results = await Promise.all(
      condition.OR.map((sub) => evaluateCondition(sub, query, context))
    );
  ...
  if (condition.entity_present && condition.entity_present.length > 0) {
    ...
    const hasMatchingEntity = entities.data.some(
      (e2) => condition.entity_present.some(
        (type) => e2.entityType.toLowerCase() === type.toLowerCase()
      )
    );
...
function cosineSimilarity(a2, b) {
  if (a2.length !== b.length || a2.length === 0) return 0;
  let dot = 0;
  let normA = 0;
  let normB = 0;
...
var init_trigger_matcher = __esm({
  "packages/engine/src/retrieval/trigger-matcher.ts"() {
    "use strict";
    init_vector_search();
  }
...
// packages/engine/src/retrieval/salience-scorer.ts
function scoreSalience(candidates, config) {
  const halfLifeDays = config?.halfLifeDays ?? 30;
  const normalizationK = config?.normalizationK ?? 50;
  return candidates.map((candidate) => {
    const { fact } = candidate;
    const lambda = Math.LN2 / halfLifeDays;
    const daysSinceAccess = fact.lastAccessed ? (Date.now() - new Date(fact.lastAccessed).getTime()) / (1e3 * 60 * 60 * 24) : Infinity;
    const accessRecency = fact.lastAccessed ? Math.exp(-lambda * daysSinceAccess) : 0;
    const daysSinceCreation = fact.createdAt ? (Date.now() - new Date(fact.createdAt).getTime()) / (1e3 * 60 * 60 * 24) : Infinity;
    ...
    const frequencyFactor = Math.min(1, Math.log(1 + fact.frequency) / Math.log(1 + normalizationK));
...
var init_salience_scorer = __esm({
  "packages/engine/src/retrieval/salience-scorer.ts"() {
    "use strict";
  }
});

// packages/engine/src/retrieval/fusion.ts
function fuseAndRank(candidates, weights, limit) {
  if (candidates.length === 0) return [];
  const sum = weights.vector + weights.keyword + weights.graph + weights.recency + weights.salience + weights.temporal;
  const w = sum === 0 ? { vector: 1 / 6, keyword: 1 / 6, graph: 1 / 6, recency: 1 / 6, salience: 1 / 6, temporal: 1 / 6 } : {
  ...
  for (const c2 of candidates) {
    const existing = factMap.get(c2.fact.id);
    if (existing) {
      existing.vectorScore = Math.max(existing.vectorScore, c2.vectorScore);
      existing.keywordScore = Math.max(existing.keywordScore, c2.keywordScore);
      existing.graphScore = Math.max(existing.graphScore, c2.graphScore);
      existing.recencyScore = Math.max(existing.recencyScore, c2.recencyScore);
      existing.salienceScore = Math.max(existing.salienceScore, c2.salienceScore);
      existing.temporalScore = Math.max(existing.temporalScore, c2.temporalScore);
      if (c2.triggeredBy) existing.triggeredBy = c2.triggeredBy;
    } else {
      factMap.set(c2.fact.id, {
        fact: c2.fact,
        vectorScore: c2.vectorScore,
        keywordScore: c2.keywordScore,
        graphScore: c2.graphScore,
        recencyScore: c2.recencyScore,
        salienceScore: c2.salienceScore,
        temporalScore: c2.temporalScore,
        source: c2.source,
        triggeredBy: c2.triggeredBy
      });
    }
  }
  const results = [];
  for (const entry of factMap.values()) {
    const score = entry.vectorScore * w.vector + entry.keywordScore * w.keyword + entry.graphScore * w.graph + entry.recencyScore * w.recency + entry.salienceScore * w.salience + entry.temporalScore * w.temporal;
    results.push({
      fact: entry.fact,
      score,
      signals: {
...
// packages/engine/src/retrieval/contradiction-surfacer.ts
async function surfaceContradictions(storage, tenantId, results) {
  const enriched = [];
  for (const result of results) {
    const { fact } = result;
    let contradiction;
    if (fact.contradictionStatus !== "none" && fact.contradictsId) {
...
function buildTimeline(oldValidFrom, _oldValidUntil, newValidFrom) {
  const oldDate = new Date(oldValidFrom);
  const newDate = new Date(newValidFrom);
  const diffMs = newDate.getTime() - oldDate.getTime();
  const diffDays = Math.abs(Math.round(diffMs / (1e3 * 60 * 60 * 24)));
...
var init_contradiction_surfacer = __esm({
  "packages/engine/src/retrieval/contradiction-surfacer.ts"() {
    "use strict";
  }
});

// packages/engine/src/salience/decay.ts
function calculateDecayScore(input) {
  ...
  const frequencyFactor = Math.min(1, Math.log(1 + frequency) / Math.log(1 + normalizationK));
...
// packages/engine/src/feedback/tracker.ts
async function recordAccesses(storage, tenantId, query, results, config) {
  const halfLifeDays = config?.halfLifeDays ?? 30;
  const normalizationK = config?.normalizationK ?? 50;
  const decayUpdates = [];
  for (let i3 = 0; i3 < results.length; i3++) {
    ...
    await storage.createMemoryAccess({
      ...
      similarityScore: result.signals.vectorScore > 0 ? result.signals.vectorScore : void 0,
      rankPosition: i3 + 1,
...
// packages/engine/src/retrieval/embedding-cache.ts
async function hashText(text) {
  const encoded = new TextEncoder().encode(text);
  const hash = await crypto.subtle.digest("SHA-256", encoded);
  return Array.from(new Uint8Array(hash)).map((b) => b.toString(16).padStart(2, "0")).join("").slice(0, 16);
}
var CachedEmbeddingAdapter;
var init_embedding_cache = __esm({
  "packages/engine/src/retrieval/embedding-cache.ts"() {
    "use strict";
    CachedEmbeddingAdapter = class {
      constructor(inner, cache2, ttlSeconds = 3600) {
        this.inner = inner;
        this.cache = cache2;
      ...
      async embedBatch(texts) {
        ...
        for (let i3 = 0; i3 < texts.length; i3++) {
          const key = `emb:${this.model}:${await hashText(texts[i3])}`;
          const cached = await this.cache.get(key);
          results.push(cached);
          if (!cached) uncachedIndices.push(i3);
        }
        if (uncachedIndices.length > 0) {
          const uncachedTexts = uncachedIndices.map((i3) => texts[i3]);
          const freshEmbeddings = await this.inner.embedBatch(uncachedTexts);
          for (let j2 = 0; j2 < uncachedIndices.length; j2++) {
            const idx = uncachedIndices[j2];
            results[idx] = freshEmbeddings[j2];
            const key = `emb:${this.model}:${await hashText(texts[idx])}`;
            await this.cache.set(key, freshEmbeddings[j2], this.ttlSeconds);
          }
        }
        return results;
      }
...
// packages/engine/src/retrieval/temporal-scorer.ts
function extractTimeReference(query) {
  const lower = query.toLowerCase();
  let ordering;
  if (/\bfirst\b/.test(lower)) ordering = "first";
  if (/\blast\b/.test(lower) && !/\blast\s+(week|month|year)\b/.test(lower)) ordering = "last";
  for (let i3 = 0; i3 < MONTH_NAMES.length; i3++) {
    const name = MONTH_NAMES[i3];
    const abbr = MONTH_ABBR[i3];
    const monthPattern = new RegExp(`(?:${name}|${abbr})\\.?\\s+(\\d{1,2})(?:st|nd|rd|th)?(?:[,\\s]+(\\d{4}))?`, "i");
    const match2 = lower.match(monthPattern);
    if (match2) {
      return { month: i3, day: parseInt(match2[1], 10), year: match2[2] ? parseInt(match2[2], 10) : void 0, ordering };
    }
    const reversePattern = new RegExp(`(\\d{1,2})(?:st|nd|rd|th)?\\s+(?:${name}|${abbr})(?:[,\\s]+(\\d{4}))?`, "i");
    const revMatch = lower.match(reversePattern);
    if (revMatch) {
      return { month: i3, day: parseInt(revMatch[1], 10), year: revMatch[2] ? parseInt(revMatch[2], 10) : void 0, ordering };
    }
  }
  for (let i3 = 0; i3 < MONTH_NAMES.length; i3++) {
    if (lower.includes(MONTH_NAMES[i3]) || lower.includes(MONTH_ABBR[i3])) {
      return { month: i3, ordering };
    }
  }
  if (/\byesterday\b/.test(lower)) {
    const d2 = /* @__PURE__ */ new Date();
    d2.setDate(d2.getDate() - 1);
    return { year: d2.getFullYear(), month: d2.getMonth(), day: d2.getDate(), ordering };
  }
  ...
  if (/\blast\s+month\b/.test(lower)) {
    ...
    return { year: d2.getFullYear(), month: d2.getMonth(), ordering };
  }
  if (ordering) return { ordering };
  return null;
}
function scoreTemporalRelevance(candidates, timeRef) {
  if (timeRef.ordering && !timeRef.month && !timeRef.day && !timeRef.year) {
    const withDates = candidates.filter((c2) => c2.fact.eventDate != null);
    if (withDates.length === 0) return;
    withDates.sort((a2, b) => {
      const aDate = new Date(a2.fact.eventDate).getTime();
      const bDate = new Date(b.fact.eventDate).getTime();
      return timeRef.ordering === "first" ? aDate - bDate : bDate - aDate;
    });
    for (let i3 = 0; i3 < withDates.length; i3++) {
      withDates[i3].temporalScore = 1 - i3 / withDates.length;
    }
    return;
  }
  ...
  let maxDistance = 0;
  for (const c2 of candidates) {
    const eventDate = c2.fact.eventDate ?? c2.fact.documentDate;
  ...
  for (const c2 of candidates) {
    ...
    const dist = Math.abs(new Date(eventDate).getTime() - refTime);
    c2.temporalScore = 1 - dist / maxDistance;
  }
}
var MONTH_NAMES, MONTH_ABBR;
var init_temporal_scorer = __esm({
  "packages/engine/src/retrieval/temporal-scorer.ts"() {
    "use strict";
    MONTH_NAMES = [
      "january",
...
// packages/engine/src/retrieval/search.ts
var search_exports = {};
...
async function search(config, options) {
  ...
  const effectiveEmbedding = config.cache ? new CachedEmbeddingAdapter(config.embedding, config.cache) : config.embedding;
  const t0 = Date.now();
  const tCompound = Date.now();
  const compoundPromise = compoundSearchSignal(config.storage, effectiveEmbedding, options.query, options.tenantId, options.scope, options.scopeId, limit * fetchMultiplier).catch(() => ({ vectorCandidates: [], keywordCandidates: [] }));
  const tGraph = Date.now();
  const graphPromise = graphSearch(config.storage, effectiveEmbedding, options.query, options.tenantId, options.scope, options.scopeId, limit * fetchMultiplier, { maxDepth: config.graphMaxDepth, maxEntities: config.graphMaxEntities, asOf: options.temporalFilter?.asOf }).catch(() => []);
  const tTrigger = Date.now();
  const triggerPromise = matchTriggers(config.storage, effectiveEmbedding, options.query, options.tenantId, options.scope, options.scopeId).catch(() => ({ candidates: [], triggersMatched: [] }));
  const [compoundResult, graphSettled, triggerSettled] = await Promise.all([
    compoundPromise.then((r2) => {
      console.error(`[steno-search] compound: ${Date.now() - tCompound}ms (vec=${r2.vectorCandidates.length}, kw=${r2.keywordCandidates.length})`);
      return r2;
    }),
    graphPromise.then((r2) => {
      console.error(`[steno-search] graph: ${Date.now() - tGraph}ms (${Array.isArray(r2) ? r2.length : 0} candidates)`);
      return r2;
    }),
    triggerPromise.then((r2) => {
      console.error(`[steno-search] trigger: ${Date.now() - tTrigger}ms`);
      return r2;
    })
  ]);
  ...
  const candidateEmbeddings = /* @__PURE__ */ new Map();
  if (allPreRerankCandidates.length > 10) {
    try {
      const uniqueTexts = /* @__PURE__ */ new Map();
      for (const c2 of allPreRerankCandidates) {
        if (!uniqueTexts.has(c2.fact.id)) uniqueTexts.set(c2.fact.id, c2.fact.content);
      }
  ...
  if (queryEmbedding && vectorCandidates.length > 1) {
    const RERANK_W = 0.4;
    for (const c2 of vectorCandidates) {
      const factEmb = candidateEmbeddings.get(c2.fact.id);
      if (factEmb) {
        const sim = cosineSim(queryEmbedding, factEmb);
        c2.vectorScore = c2.vectorScore * (1 - RERANK_W) + sim * RERANK_W;
      }
    }
    vectorCandidates.sort((a2, b) => b.vectorScore - a2.vectorScore);
  }
  if (queryEmbedding && graphCandidates.length > 1) {
    const RERANK_W = 0.4;
    for (const c2 of graphCandidates) {
      const factEmb = candidateEmbeddings.get(c2.fact.id);
      if (factEmb) {
        const sim = cosineSim(queryEmbedding, factEmb);
        c2.graphScore = c2.graphScore * (1 - RERANK_W) + sim * RERANK_W;
      }
    }
    graphCandidates.sort((a2, b) => b.graphScore - a2.graphScore);
  }
  ...
  if (!options.includeHistory) {
    const lineageSeen = /* @__PURE__ */ new Map();
    dedupedResults = fusionResults.filter((r2, idx) => {
      const lid = r2.fact.lineageId;
      if (!lid) return true;
  ...
  const updatedFactIds = /* @__PURE__ */ new Set();
  for (const r2 of dedupedResults) {
    const meta = r2.fact.metadata;
    if (meta?.relationType === "updates" && meta?.relatedFactId) {
      updatedFactIds.add(meta.relatedFactId);
    }
  }
  if (updatedFactIds.size > 0) {
    dedupedResults = dedupedResults.filter((r2) => !updatedFactIds.has(r2.fact.id));
  }
  if (options.tokenBudget && options.tokenBudget > 0) {
    let tokenCount = 0;
    const budgetResults = [];
    for (const r2 of dedupedResults) {
      const factTokens = Math.ceil(r2.fact.content.length / 4) + (r2.fact.sourceChunk ? Math.ceil(r2.fact.sourceChunk.length / 4) : 0);
      if (tokenCount + factTokens > options.tokenBudget) break;
      tokenCount += factTokens;
      budgetResults.push(r2);
    }
  ...
  if (options.includeGraph) {
    for (const result of results) {
      ...
      for (const entity2 of entities) {
        const entityEdges = await config.storage.getEdgesForEntity(options.tenantId, entity2.id);
  ...
  if (options.includeHistory) {
    for (const result of results) {
      ...
      result.history = history.filter((f3) => f3.id !== result.fact.id).sort((a2, b) => a2.version - b.version);
    }
  }
...
var init_search = __esm({
  "packages/engine/src/retrieval/search.ts"() {
    "use strict";
    init_types();
    init_compound_search();
    init_graph_traversal();
    init_trigger_matcher();
    init_salience_scorer();
    init_fusion();
    init_contradiction_surfacer();
    init_tracker();
    init_embedding_cache();
    init_temporal_scorer();
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/API/world.js
var methods, model, compute, hooks, world_default;
var init_world = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/API/world.js"() {
    ...
    compute = {};
    hooks = [];
    world_default = { methods, model, compute, hooks };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/API/methods/compute.js
var isArray, fns, compute_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/API/methods/loops.js
var forEach, map, filter, find, some, random, loops_default;
var init_loops = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/API/methods/loops.js"() {
    ...
    loops_default = { forEach, map, filter, find, some, random };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/API/methods/utils.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/API/methods/index.js
var methods2, methods_default;
var init_methods = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/API/methods/index.js"() {
    init_compute();
    init_loops();
    init_utils();
    methods2 = Object.assign({}, utils_default, compute_default, loops_default);
    methods2.get = methods2.eq;
    methods_default = methods2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/API/View.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/_version.js
var version_default;
var init_version = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/_version.js"() {
    version_default = "14.15.0";
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/API/extend.js
...
var isObject, isArray2, addIrregulars, extend, extend_default;
var init_extend = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/API/extend.js"() {
    ...
    addIrregulars = function(model5, conj) {
    ...
    extend = function(plugin5, world2, View2, nlp2) {
      if (isArray2(plugin5)) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/API/_lib.js
var verbose;
var init_lib = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/API/_lib.js"() {
    verbose = function(set) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/API/inputs.js
var isObject2, isArray3, fromJson, preTokenized, inputs, inputs_default;
var init_inputs = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/API/inputs.js"() {
    isObject2 = (val) => {
      return Object.prototype.toString.call(val) === "[object Object]";
    };
    isArray3 = function(arr) {
      return Object.prototype.toString.call(arr) === "[object Array]";
    };
    ...
    inputs = function(input, View2, world2) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/nlp.js
var world, nlp, nlp_default;
var init_nlp = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/nlp.js"() {
    init_View();
    init_world();
    ...
    init_lib();
    init_inputs();
    world = Object.assign({}, world_default);
    nlp = function(input, lex) {
      if (lex) {
        nlp.addWords(lex);
      }
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/cache/methods/cacheDoc.js
var createCache, cacheDoc_default;
var init_cacheDoc = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/cache/methods/cacheDoc.js"() {
    createCache = function(document) {
    ...
    cacheDoc_default = createCache;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/cache/methods/index.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/cache/api.js
var methods3, addAPI, api_default;
var init_api = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/cache/api.js"() {
    methods3 = {
      /** */
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/cache/compute.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/cache/plugin.js
var plugin_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/case.js
var case_default;
var init_case = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/case.js"() {
    case_default = {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/lib/insert.js
var isTitleCase, toTitleCase, toLowerCase, spliceArr, endSpace, movePunct, moveTitleCase, cleanPrepend, cleanAppend;
var init_insert = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/lib/insert.js"() {
    isTitleCase = (str) => new RegExp("^\\p{Lu}[\\p{Ll}'\u2019]", "u").test(str) || new RegExp("^\\p{Lu}$", "u").test(str);
    toTitleCase = (str) => str.replace(new RegExp("^\\p{Ll}", "u"), (x) => x.toUpperCase());
    toLowerCase = (str) => str.replace(new RegExp("^\\p{Lu}", "u"), (x) => x.toLowerCase());
    spliceArr = (parent, index3, child) => {
    ...
    moveTitleCase = function(home, start2, needle) {
    ...
    cleanPrepend = function(home, ptr, needle, document) {
      const [n3, start2, end2] = ptr;
      if (start2 === 0) {
        endSpace(needle);
    ...
    cleanAppend = function(home, ptr, needle, document) {
      const [n3, , end2] = ptr;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/compute/uuid.js
var index, pad3, toId, uuid_default;
var init_uuid = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/compute/uuid.js"() {
    index = 0;
    pad3 = (str) => {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/insert.js
var expand, isArray4, addIds, getTerms, insert, fns2, insert_default;
var init_insert2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/insert.js"() {
    ...
    isArray4 = (arr) => Object.prototype.toString.call(arr) === "[object Array]";
    addIds = function(terms) {
    ...
    getTerms = function(input, world2) {
      const { methods: methods17 } = world2;
      if (typeof input === "string") {
    ...
    insert = function(input, view, prepend) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/replace.js
var dollarStub, fns3, isTitleCase2, toTitleCase2, toLowerCase2, replaceByFn, subDollarSign, replace_default;
var init_replace = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/replace.js"() {
    dollarStub = /\$[0-9a-z]+/g;
    fns3 = {};
    isTitleCase2 = (str) => new RegExp("^\\p{Lu}[\\p{Ll}'\u2019]", "u").test(str) || new RegExp("^\\p{Lu}$", "u").test(str);
    toTitleCase2 = (str) => str.replace(new RegExp("^\\p{Ll}", "u"), (x) => x.toUpperCase());
    toLowerCase2 = (str) => str.replace(new RegExp("^\\p{Lu}", "u"), (x) => x.toLowerCase());
    replaceByFn = function(main2, fn, keep9) {
      main2.forEach((m3) => {
        const out2 = fn(m3);
    ...
    subDollarSign = function(input, main2) {
      if (typeof input !== "string") {
        return input;
      }
    ...
    replace_default = fns3;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/lib/remove.js
var repairPunct, pluckOut, remove_default;
var init_remove = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/lib/remove.js"() {
    repairPunct = function(terms, len) {
    ...
    pluckOut = function(document, nots) {
      nots.forEach((ptr) => {
        const [n3, start2, end2] = ptr;
    ...
    remove_default = pluckOut;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/remove.js
var fixPointers, methods4, remove_default2;
var init_remove2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/remove.js"() {
    init_remove();
    fixPointers = function(ptrs, gonePtrs) {
      ptrs = ptrs.map((ptr) => {
        const [n3] = ptr;
        if (!gonePtrs[n3]) {
          return ptr;
    ...
    remove_default2 = methods4;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/whitespace.js
var methods5, whitespace_default;
var init_whitespace = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/whitespace.js"() {
    ...
    whitespace_default = methods5;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/lib/_sort.js
var alpha, length, wordCount, sequential, byFreq, sort_default;
var init_sort = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/lib/_sort.js"() {
    alpha = (a2, b) => {
    ...
    sort_default = { alpha, length, wordCount, sequential, byFreq };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/sort.js
var seqNames, freqNames, alphaNames, customSort, sort, reverse, unique, sort_default2;
var init_sort2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/sort.js"() {
    init_sort();
    seqNames = /* @__PURE__ */ new Set(["index", "sequence", "seq", "sequential", "chron", "chronological"]);
    freqNames = /* @__PURE__ */ new Set(["freq", "frequency", "topk", "repeats"]);
    alphaNames = /* @__PURE__ */ new Set(["alpha", "alphabetical"]);
    customSort = function(view, fn) {
    ...
    sort_default2 = { unique, reverse, sort };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/concat.js
var isArray5, combineDocs, combineViews, concat_default;
var init_concat = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/concat.js"() {
    isArray5 = (arr) => Object.prototype.toString.call(arr) === "[object Array]";
    combineDocs = function(homeDocs, inputDocs) {
    ...
    combineViews = function(home, input) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/harden.js
var harden, soften, harden_default;
var init_harden = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/harden.js"() {
    harden = function() {
    ...
    harden_default = { harden, soften };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/index.js
var methods6, addAPI2, api_default2;
var init_api2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/api/index.js"() {
    init_case();
    init_insert2();
    ...
    init_concat();
    init_harden();
    methods6 = Object.assign({}, case_default, insert_default, replace_default, remove_default2, whitespace_default, sort_default2, concat_default, harden_default);
    addAPI2 = function(View2) {
      Object.assign(View2.prototype, methods6);
    };
    api_default2 = addAPI2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/compute/index.js
var compute2, compute_default3;
var init_compute3 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/compute/index.js"() {
    ...
    compute_default3 = compute2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/change/plugin.js
var plugin_default2;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/model/contractions.js
var contractions_default;
var init_contractions = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/model/contractions.js"() {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/model/number-suffix.js
var t, number_suffix_default;
var init_number_suffix = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/model/number-suffix.js"() {
    t = true;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/model/index.js
var model_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/compute/contractions/_splice.js
var insertContraction, splice_default;
var init_splice = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/compute/contractions/_splice.js"() {
    insertContraction = function(document, point, words) {
      const [n3, w] = point;
    ...
    splice_default = insertContraction;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/compute/contractions/apostrophe-d.js
var hasContraction, alwaysDid, useWould, useHad, _apostropheD, apostrophe_d_default;
var init_apostrophe_d = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/compute/contractions/apostrophe-d.js"() {
    hasContraction = /'/;
    alwaysDid = /* @__PURE__ */ new Set([
      "what",
      "how",
      "when",
      "where",
    ...
    useHad = /* @__PURE__ */ new Set([
      "been",
      "gone"
    ]);
    _apostropheD = function(terms, i3) {
    ...
    apostrophe_d_default = _apostropheD;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/compute/contractions/apostrophe-t.js
var apostropheT, apostrophe_t_default;
var init_apostrophe_t = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/compute/contractions/apostrophe-t.js"() {
    apostropheT = function(terms, i3) {
    ...
    apostrophe_t_default = apostropheT;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/compute/contractions/french.js
var hasContraction2, isFeminine, isMasculine, preL, preD, preJ, french_default;
var init_french = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/compute/contractions/french.js"() {
    hasContraction2 = /'/;
    isFeminine = /(e|é|aison|sion|tion)$/;
    isMasculine = /(age|isme|acle|ege|oire)$/;
    preL = (terms, i3) => {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/compute/contractions/number-range.js
var isRange, timeRange, phoneNum, numberRange, number_range_default;
var init_number_range = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/compute/contractions/number-range.js"() {
    isRange = /^([0-9.]{1,4}[a-z]{0,2}) ?[-–—] ?([0-9]{1,4}[a-z]{0,2})$/i;
    timeRange = /^([0-9]{1,2}(:[0-9][0-9])?(am|pm)?) ?[-–—] ?([0-9]{1,2}(:[0-9][0-9])?(am|pm)?)$/i;
    phoneNum = /^[0-9]{3}-[0-9]{4}$/;
    numberRange = function(terms, i3) {
      const term = terms[i3];
    ...
    number_range_default = numberRange;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/compute/contractions/number-unit.js
var numUnit, numberUnit, number_unit_default;
var init_number_unit = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/compute/contractions/number-unit.js"() {
    numUnit = /^([+-]?[0-9][.,0-9]*)([a-z°²³µ/]+)$/;
    numberUnit = function(terms, i3, world2) {
    ...
    number_unit_default = numberUnit;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/compute/contractions/index.js
var byApostrophe, numDash, reTag, byEnd, byStart, knownOnes, toDocs, thereHas, contractions, contractions_default2;
var init_contractions2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/compute/contractions/index.js"() {
    init_splice();
    init_apostrophe_d();
    init_apostrophe_t();
    ...
    byApostrophe = /'/;
    numDash = /^[0-9][^-–—]*[-–—].*?[0-9]/;
    reTag = function(terms, view, start2, len) {
    ...
    knownOnes = function(list4, term, before2, after2) {
    ...
    toDocs = function(words, view) {
      const doc = view.fromText(words.join(" "));
      doc.compute(["id", "alias"]);
      return doc.docs[0];
    };
    thereHas = function(terms, i3) {
    ...
    contractions_default2 = contractions;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/compute/index.js
var compute_default4;
var init_compute4 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/compute/index.js"() {
    init_contractions2();
    compute_default4 = { contractions: contractions_default2 };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/plugin.js
var plugin, plugin_default3;
var init_plugin3 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/contraction-one/plugin.js"() {
    ...
    plugin_default3 = plugin;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/freeze/compute.js
var freeze, unfreeze, compute_default5;
var init_compute5 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/freeze/compute.js"() {
    freeze = function(view) {
    ...
    compute_default5 = { frozen: freeze, freeze, unfreeze };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/freeze/debug.js
var blue, dim, debug, debug_default;
var init_debug = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/freeze/debug.js"() {
    blue = (str) => "\x1B[34m" + str + "\x1B[0m";
    dim = (str) => "\x1B[3m\x1B[2m" + str + "\x1B[0m";
    debug = function(view) {
      view.docs.forEach((terms) => {
        console.log(blue("\n  \u250C\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500"));
        terms.forEach((t3) => {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/freeze/plugin.js
var plugin_default4;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lexicon/compute/multi-word.js
var multiWord, multi_word_default;
var init_multi_word = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lexicon/compute/multi-word.js"() {
    multiWord = function(terms, start_i, world2) {
      const { model: model5, methods: methods17 } = world2;
      const setTag2 = methods17.one.setTag;
      const multi = model5.one._multiCache || {};
    ...
    multi_word_default = multiWord;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lexicon/compute/single-word.js
var prefix, allowPrefix, checkLexicon, single_word_default;
var init_single_word = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lexicon/compute/single-word.js"() {
    prefix = /^(under|over|mis|re|un|dis|semi|pre|post)-?/;
    allowPrefix = /* @__PURE__ */ new Set(["Verb", "Infinitive", "PastTense", "Gerund", "PresentTense", "Adjective", "Participle"]);
    checkLexicon = function(terms, i3, world2) {
      const { model: model5, methods: methods17 } = world2;
      const setTag2 = methods17.one.setTag;
      const { lexicon: lexicon4 } = model5.one;
      const t3 = terms[i3];
    ...
    single_word_default = checkLexicon;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lexicon/compute/index.js
var lexicon, compute_default6;
var init_compute6 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lexicon/compute/index.js"() {
    init_multi_word();
    init_single_word();
    lexicon = function(view) {
      const world2 = view.world;
      view.docs.forEach((terms) => {
        for (let i3 = 0; i3 < terms.length; i3 += 1) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lexicon/methods/expand.js
var expand2, expand_default;
var init_expand = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lexicon/methods/expand.js"() {
    expand2 = function(words) {
      const lex = {};
      const _multi = {};
    ...
    expand_default = expand2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lexicon/methods/index.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lexicon/lib.js
var addWords, lib_default;
var init_lib2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lexicon/lib.js"() {
    addWords = function(words, isFrozen = false) {
    ...
    lib_default = { addWords };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lexicon/plugin.js
var model2, plugin_default5;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lookup/api/buildTrie/index.js
var tokenize, buildTrie, buildTrie_default;
var init_buildTrie = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lookup/api/buildTrie/index.js"() {
    tokenize = function(phrase, world2) {
      const { methods: methods17, model: model5 } = world2;
    ...
    buildTrie = function(phrases, world2) {
      const goNext = [{}];
      const endAs = [null];
      const failTo = [0];
      const xs = [];
    ...
    buildTrie_default = buildTrie;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lookup/api/scan.js
var scanWords, cacheMiss, scan, scan_default;
var init_scan = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lookup/api/scan.js"() {
    scanWords = function(terms, trie, opts2) {
      let n3 = 0;
      const results = [];
    ...
    cacheMiss = function(words, cache2) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lookup/api/index.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lookup/api/buildTrie/compress.js
var truncate, compress, compress_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/lookup/plugin.js
var lib, plugin_default6;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/api/_lib.js
var relPointer, fixPointers2, parseRegs, isObject4, isView, isNet;
var init_lib3 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/api/_lib.js"() {
    relPointer = function(ptrs, parent) {
      if (!parent) {
        return ptrs;
      }
      ptrs.forEach((ptr) => {
    ...
    fixPointers2 = function(res, parent) {
      let { ptrs } = res;
      const { byGroup } = res;
    ...
    parseRegs = function(regs, opts2, world2) {
    ...
    isView = (val) => val && isObject4(val) && val.isView === true;
    isNet = (val) => val && isObject4(val) && val.isNet === true;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/api/match.js
var match, matchOne, has, ifFn, ifNo, match_default;
var init_match = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/api/match.js"() {
    init_lib3();
    match = function(regs, group, opts2) {
    ...
    matchOne = function(regs, group, opts2) {
      const one = this.methods.one;
      if (isView(regs)) {
    ...
    ifFn = function(regs, group, opts2) {
      const one = this.methods.one;
      if (isView(regs)) {
    ...
    ifNo = function(regs, group, opts2) {
      const { methods: methods17 } = this;
    ...
    match_default = { matchOne, match, has, if: ifFn, ifNo };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/api/lookaround.js
var before, after, growLeft, growRight, grow, lookaround_default;
var init_lookaround = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/api/lookaround.js"() {
    before = function(regs, group, opts2) {
      const { indexN: indexN2 } = this.methods.one.pointer;
      const pre = [];
    ...
    after = function(regs, group, opts2) {
      const { indexN: indexN2 } = this.methods.one.pointer;
      const post = [];
      const byN = indexN2(this.fullPointer);
    ...
    growLeft = function(regs, group, opts2) {
      if (typeof regs === "string") {
    ...
    growRight = function(regs, group, opts2) {
      if (typeof regs === "string") {
        regs = this.world.methods.one.parseMatch(regs, opts2, this.world);
      }
      regs[0].start = true;
    ...
    grow = function(regs, group, opts2) {
      return this.growRight(regs, group, opts2).growLeft(regs, group, opts2);
    };
    lookaround_default = { before, after, growLeft, growRight, grow };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/api/split.js
var combine, isArray6, getDoc, addIds2, methods7, split_default;
var init_split = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/api/split.js"() {
    combine = function(left, right) {
      return [left[0], left[1], right[2]];
    };
    isArray6 = function(arr) {
      return Object.prototype.toString.call(arr) === "[object Array]";
    };
    getDoc = (reg, view, group) => {
    ...
    split_default = methods7;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/api/join.js
var isNeighbour, mergeIf, methods8, join_default;
var init_join = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/api/join.js"() {
    isNeighbour = function(ptrL, ptrR) {
      if (!ptrL || !ptrR) {
        return false;
      }
      if (ptrL[0] !== ptrR[0]) {
    ...
    mergeIf = function(doc, lMatch, rMatch) {
    ...
    join_default = methods8;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/api/index.js
var methods9, matchAPI, api_default4;
var init_api4 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/api/index.js"() {
    init_match();
    init_lookaround();
    init_split();
    init_join();
    methods9 = Object.assign({}, match_default, lookaround_default, split_default, join_default);
    ...
    matchAPI = function(View2) {
      Object.assign(View2.prototype, methods9);
    };
    api_default4 = matchAPI;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/parseMatch/01-parseBlocks.js
var bySlashes, byParentheses, byWord, isBlock, isReg, cleanUp, parseBlocks, parseBlocks_default;
var init_parseBlocks = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/parseMatch/01-parseBlocks.js"() {
    bySlashes = /(?:^|\s)([![^]*(?:<[^<]*>)?\/.*?[^\\/]\/[?\]+*$~]*)(?:\s|$)/;
    byParentheses = /([!~[^]*(?:<[^<]*>)?\([^)]+[^\\)]\)[?\]+*$~]*)(?:\s|$)/;
    byWord = / /g;
    ...
    parseBlocks_default = parseBlocks;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/parseMatch/02-parseToken.js
var hasMinMax, andSign, captureName, titleCase, end, start, stripStart, stripEnd, stripBoth, parseToken, parseToken_default;
var init_parseToken = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/parseMatch/02-parseToken.js"() {
    hasMinMax = /\{([0-9]+)?(, *[0-9]*)?\}/;
    andSign = /&&/;
    captureName = new RegExp(/^<\s*(\S+)\s*>/);
    titleCase = (str) => str.charAt(0).toUpperCase() + str.substring(1);
    end = (str) => str.charAt(str.length - 1);
    start = (str) => str.charAt(0);
    stripStart = (str) => str.substring(1);
    stripEnd = (str) => str.substring(0, str.length - 1);
    stripBoth = function(str) {
      str = stripStart(str);
      str = stripEnd(str);
      return str;
    };
    parseToken = function(w, opts2) {
      const obj = {};
    ...
    parseToken_default = parseToken;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/parseMatch/03-splitHyphens.js
var hasDash, splitHyphens, splitHyphens_default;
var init_splitHyphens = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/parseMatch/03-splitHyphens.js"() {
    hasDash = /[a-z0-9][-–—][a-z]/i;
    splitHyphens = function(regs, world2) {
    ...
    splitHyphens_default = splitHyphens;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/parseMatch/04-inflect-root.js
var addVerbs, addNoun, addAdjective, inflectRoot, inflect_root_default;
var init_inflect_root = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/parseMatch/04-inflect-root.js"() {
    addVerbs = function(token, world2) {
    ...
    addAdjective = function(token, world2) {
    ...
    inflectRoot = function(regs, world2) {
      regs = regs.map((token) => {
        if (token.root) {
    ...
    inflect_root_default = inflectRoot;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/parseMatch/05-postProcess.js
var nameGroups, doFastOrMode, fuzzyOr, postProcess, postProcess_default;
var init_postProcess = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/parseMatch/05-postProcess.js"() {
    nameGroups = function(regs) {
      let index3 = 0;
      let inGroup = null;
    ...
    postProcess = function(regs) {
      regs = nameGroups(regs);
      regs = doFastOrMode(regs);
      regs = fuzzyOr(regs);
      return regs;
    };
    postProcess_default = postProcess;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/parseMatch/index.js
var syntax, parseMatch_default;
var init_parseMatch = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/parseMatch/index.js"() {
    init_parseBlocks();
    init_parseToken();
    init_splitHyphens();
    init_inflect_root();
    init_postProcess();
    syntax = function(input, opts2, world2) {
    ...
    parseMatch_default = syntax;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/01-failFast.js
var anyIntersection, failFast, failFast_default;
var init_failFast = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/01-failFast.js"() {
    anyIntersection = function(setA, setB) {
    ...
    failFast_default = failFast;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/term/_fuzzy.js
var editDistance, fuzzyMatch, fuzzy_default;
var init_fuzzy = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/term/_fuzzy.js"() {
    editDistance = function(strA, strB) {
    ...
    fuzzyMatch = function(strA, strB, minLength = 3) {
      if (strA === strB) {
        return 1;
      }
    ...
    fuzzy_default = fuzzyMatch;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/termMethods.js
var startQuote, endQuote, hasHyphen, hasDash2, hasPost, methods10, termMethods_default;
var init_termMethods = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/termMethods.js"() {
    startQuote = /([\u0022\uFF02\u0027\u201C\u2018\u201F\u201B\u201E\u2E42\u201A\u00AB\u2039\u2035\u2036\u2037\u301D\u0060\u301F])/;
    endQuote = /([\u0022\uFF02\u0027\u201D\u2019\u00BB\u203A\u2032\u2033\u2034\u301E\u00B4])/;
    hasHyphen = /^[-–—]$/;
    hasDash2 = / [-–—]{1,3} /;
    hasPost = (term, punct) => term.post.indexOf(punct) !== -1;
    methods10 = {
    ...
    termMethods_default = methods10;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/term/doesMatch.js
var wrapMatch, doesMatch, doesMatch_default;
var init_doesMatch = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/term/doesMatch.js"() {
    init_fuzzy();
    init_termMethods();
    wrapMatch = function() {
    };
    doesMatch = function(term, reg, index3, length2) {
    ...
    doesMatch_default = wrapMatch;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/logic/greedy.js
var getGreedy, greedyTo, isEndGreedy;
var init_greedy = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/logic/greedy.js"() {
    init_doesMatch();
    getGreedy = function(state, endReg) {
    ...
    greedyTo = function(state, nextReg) {
      let t3 = state.t;
      if (!nextReg) {
    ...
    isEndGreedy = function(reg, state) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/_lib.js
var getGroup;
var init_lib4 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/_lib.js"() {
    getGroup = function(state, term_index) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/astrix.js
var doAstrix, astrix_default;
var init_astrix = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/astrix.js"() {
    init_greedy();
    init_lib4();
    doAstrix = function(state) {
      const { regs } = state;
    ...
    astrix_default = doAstrix;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/logic/and-or.js
var isArray7, doOrBlock, doAndBlock;
var init_and_or = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/logic/and-or.js"() {
    init_doesMatch();
    isArray7 = function(arr) {
      return Object.prototype.toString.call(arr) === "[object Array]";
    };
    doOrBlock = function(state, skipN = 0) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/or-block.js
var orBlock, or_block_default;
var init_or_block = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/or-block.js"() {
    init_and_or();
    init_lib4();
    orBlock = function(state) {
      const { regs } = state;
      const reg = regs[state.r];
    ...
    or_block_default = orBlock;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/and-block.js
var andBlock, and_block_default;
var init_and_block = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/and-block.js"() {
    init_and_or();
    init_lib4();
    andBlock = function(state) {
      const { regs } = state;
      const reg = regs[state.r];
    ...
    and_block_default = andBlock;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/logic/negative-greedy.js
var negGreedy, negative_greedy_default;
var init_negative_greedy = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/logic/negative-greedy.js"() {
    init_doesMatch();
    negGreedy = function(state, reg, nextReg) {
      let skip = 0;
    ...
    negative_greedy_default = negGreedy;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/negative.js
var doNegative, negative_default;
var init_negative = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/negative.js"() {
    init_doesMatch();
    init_negative_greedy();
    doNegative = function(state) {
      const { regs } = state;
      const reg = regs[state.r];
    ...
    negative_default = doNegative;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/optional-match.js
var foundOptional, optional_match_default;
var init_optional_match = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/optional-match.js"() {
    init_doesMatch();
    foundOptional = function(state) {
      const { regs } = state;
      const reg = regs[state.r];
    ...
    optional_match_default = foundOptional;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/greedy-match.js
var greedyMatch, greedy_match_default;
var init_greedy_match = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/greedy-match.js"() {
    init_greedy();
    greedyMatch = function(state) {
    ...
    greedy_match_default = greedyMatch;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/contraction-skip.js
var contractionSkip, contraction_skip_default;
var init_contraction_skip = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/contraction-skip.js"() {
    contractionSkip = function(state) {
      const term = state.terms[state.t];
      const reg = state.regs[state.r];
    ...
    contraction_skip_default = contractionSkip;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/simple-match.js
var setGroup, simpleMatch, simple_match_default;
var init_simple_match = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/steps/simple-match.js"() {
    init_lib4();
    init_optional_match();
    init_greedy_match();
    init_contraction_skip();
    setGroup = function(state, startAt) {
      const reg = state.regs[state.r];
    ...
    simple_match_default = simpleMatch;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/02-from-here.js
var tryHere, from_here_default;
var init_from_here = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/02-from-here.js"() {
    init_astrix();
    init_or_block();
    init_and_block();
    ...
    init_greedy();
    init_doesMatch();
    tryHere = function(terms, regs, start_i, phrase_length) {
    ...
    from_here_default = tryHere;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/03-getGroup.js
var getGroup2, getGroup_default;
var init_getGroup = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/03-getGroup.js"() {
    getGroup2 = function(res, group) {
      const ptrs = [];
      const byGroup = {};
      if (res.length === 0) {
        return { ptrs, byGroup };
    ...
    getGroup_default = getGroup2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/03-notIf.js
var notIf, notIf_default;
var init_notIf = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/03-notIf.js"() {
    init_from_here();
    notIf = function(results, not, docs) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/index.js
var addSentence, handleStart, runMatch, match_default2;
var init_match2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/match/index.js"() {
    ...
    handleStart = function(terms, regs, n3) {
    ...
    runMatch = function(docs, todo, cache2) {
      cache2 = cache2 || [];
    ...
    match_default2 = runMatch;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/index.js
var methods11, methods_default4;
var init_methods4 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/methods/index.js"() {
    init_parseMatch();
    init_match2();
    init_termMethods();
    ...
    methods_default4 = methods11;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/lib.js
var lib_default2;
var init_lib5 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/lib.js"() {
    lib_default2 = {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/match/plugin.js
var plugin_default7;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/html.js
var isClass, isId, escapeXml, toTag, getIndex, html, html_default;
var init_html = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/html.js"() {
    isClass = /^\../;
    isId = /^#./;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/_text.js
var trimEnd, trimStart, punctToKill, isHyphen, hasSpace, textFromTerms, textFromDoc;
var init_text = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/_text.js"() {
    trimEnd = /[,:;)\]*.?~!\u0022\uFF02\u201D\u2019\u00BB\u203A\u2032\u2033\u2034\u301E\u00B4—-]+$/;
    trimStart = /^[(['"*~\uFF02\u201C\u2018\u201F\u201B\u201E\u2E42\u201A\u00AB\u2039\u2035\u2036\u2037\u301D\u0060\u301F]+/;
    ...
    isHyphen = /^[-–—]$/;
    hasSpace = / /;
    textFromTerms = function(terms, opts2, keepSpace = true) {
      let txt = "";
      terms.forEach((t3) => {
        let pre = t3.pre || "";
        let post = t3.post || "";
    ...
    textFromDoc = function(docs, opts2) {
      let text = "";
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/_fmts.js
var fmts, fmts_default;
var init_fmts = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/_fmts.js"() {
    fmts = {
      text: {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/methods/hash.js
var k, i, md5, hash_default;
var init_hash = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/methods/hash.js"() {
    k = [];
    i = 0;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/json.js
var defaults, opts, merge, fns4, toJSON, methods12, json_default;
var init_json = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/json.js"() {
    init_text();
    init_fmts();
    ...
    opts = { case: "none", unicode: "some", form: "machine", punctuation: "some" };
    merge = function(a2, b) {
    ...
    json_default = methods12;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/debug.js
var isClientSide, debug2, debug_default2;
var init_debug2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/debug.js"() {
    isClientSide = () => typeof window !== "undefined" && window.document;
    debug2 = function(fmt2) {
    ...
    debug_default2 = debug2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/wrap.js
var toText, findStarts, wrap, wrap_default;
var init_wrap = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/wrap.js"() {
    toText = function(term) {
      const pre = term.pre || "";
      const post = term.post || "";
      return pre + term.text + post;
    };
    findStarts = function(doc, obj) {
      const starts = {};
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/out.js
var isObject5, topk, out, methods13, out_default;
var init_out = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/out.js"() {
    init_debug2();
    init_wrap();
    ...
    out_default = methods13;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/text.js
var isObject6, text_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/index.js
var methods14, addAPI3, api_default5;
var init_api5 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/api/index.js"() {
    init_html();
    init_json();
    init_out();
    init_text2();
    methods14 = Object.assign({}, out_default, text_default, json_default, html_default);
    addAPI3 = function(View2) {
      Object.assign(View2.prototype, methods14);
    };
    api_default5 = addAPI3;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/methods/debug/client-side.js
var logClientSide, client_side_default;
var init_client_side = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/methods/debug/client-side.js"() {
    logClientSide = function(view) {
    ...
    client_side_default = logClientSide;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/methods/debug/_color.js
var reset, cli, color_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/methods/debug/tags.js
var tagString, showTags, tags_default;
var init_tags = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/methods/debug/tags.js"() {
    init_color();
    tagString = function(tags, model5) {
    ...
    tags_default = showTags;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/methods/debug/chunks.js
var showChunks, chunks_default;
var init_chunks = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/methods/debug/chunks.js"() {
    init_color();
    showChunks = function(view) {
      const { docs } = view;
      console.log("");
      docs.forEach((terms) => {
        const out2 = [];
    ...
    chunks_default = showChunks;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/methods/debug/highlight.js
var split, spliceIn, showHighlight, highlight_default;
var init_highlight = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/methods/debug/highlight.js"() {
    init_color();
    split = (txt, offset2, index3) => {
      const buff = index3 * 9;
    ...
    highlight_default = showHighlight;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/methods/debug/index.js
var debug3, debug_default3;
var init_debug3 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/methods/debug/index.js"() {
    ...
    debug_default3 = debug3;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/methods/index.js
var init_methods5 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/methods/index.js"() {
    init_hash();
    init_debug3();
  }
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/output/plugin.js
var plugin_default8;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/api/lib/_lib.js
var doesOverlap, getExtent, indexN, uniquePtrs;
var init_lib6 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/api/lib/_lib.js"() {
    doesOverlap = function(a2, b) {
      if (a2[0] !== b[0]) {
        return false;
      }
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/api/lib/split.js
var pivotBy, doesMatch2, splitAll, split_default2;
var init_split2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/api/lib/split.js"() {
    init_lib6();
    pivotBy = function(full, m3) {
    ...
    doesMatch2 = function(full, m3) {
      return full[1] <= m3[1] && m3[2] <= full[2];
    };
    splitAll = function(full, m3) {
      const byN = indexN(m3);
      const res = [];
      full.forEach((ptr) => {
        const [n3] = ptr;
    ...
    split_default2 = splitAll;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/methods/getDoc.js
var max, blindSweep, repairEnding, getDoc2, getDoc_default;
var init_getDoc = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/methods/getDoc.js"() {
    max = 20;
    blindSweep = function(id, doc, n3) {
    ...
    repairEnding = function(ptr, document) {
    ...
    getDoc2 = function(ptrs, document) {
      let doc = [];
      ptrs.forEach((ptr, i3) => {
        if (!ptr) {
          return;
    ...
    getDoc_default = getDoc2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/methods/index.js
var termList, methods_default5;
var init_methods6 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/methods/index.js"() {
    init_lib6();
    init_split2();
    init_getDoc();
    termList = function(docs) {
      const arr = [];
      for (let i3 = 0; i3 < docs.length; i3 += 1) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/api/lib/union.js
var getUnion, union_default;
var init_union = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/api/lib/union.js"() {
    init_lib6();
    getUnion = function(a2, b) {
    ...
    union_default = getUnion;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/api/lib/difference.js
var subtract, difference_default;
var init_difference = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/api/lib/difference.js"() {
    init_split2();
    subtract = function(refs, not) {
      const res = [];
    ...
    difference_default = subtract;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/api/lib/intersection.js
var intersection, getIntersection, intersection_default;
var init_intersection = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/api/lib/intersection.js"() {
    init_lib6();
    intersection = function(a2, b) {
    ...
    intersection_default = getIntersection;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/api/index.js
var isArray8, getDoc3, addIds3, methods15, addAPI4, api_default6;
var init_api6 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/api/index.js"() {
    init_union();
    init_difference();
    init_intersection();
    isArray8 = function(arr) {
      return Object.prototype.toString.call(arr) === "[object Array]";
    };
    getDoc3 = (m3, view) => {
    ...
    addAPI4 = function(View2) {
      Object.assign(View2.prototype, methods15);
    };
    api_default6 = addAPI4;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/pointers/plugin.js
var plugin_default9;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/lib.js
var lib_default3;
var init_lib7 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/lib.js"() {
    lib_default3 = {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/api.js
var api, api_default7;
var init_api7 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/api.js"() {
    api = function(View2) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/buildNet/01-parse.js
var getTokenNeeds, getNeeds, getWants, parse, parse_default;
var init_parse = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/buildNet/01-parse.js"() {
    getTokenNeeds = function(reg) {
    ...
    parse = function(matches3, world2) {
      const parseMatch = world2.methods.one.parseMatch;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/buildNet/index.js
var buildNet, buildNet_default;
var init_buildNet = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/buildNet/index.js"() {
    init_parse();
    buildNet = function(matches3, world2) {
    ...
    buildNet_default = buildNet;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/sweep/01-getHooks.js
var getHooks, getHooks_default;
var init_getHooks = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/sweep/01-getHooks.js"() {
    getHooks = function(docCaches, hooks2) {
    ...
    getHooks_default = getHooks;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/sweep/02-trim-down.js
var localTrim, trim_down_default;
var init_trim_down = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/sweep/02-trim-down.js"() {
    localTrim = function(maybeList, docCache) {
    ...
    trim_down_default = localTrim;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/sweep/04-runMatch.js
var runMatch2, runMatch_default;
var init_runMatch = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/sweep/04-runMatch.js"() {
    runMatch2 = function(maybeList, document, docCache, methods17, opts2) {
      const results = [];
    ...
    runMatch_default = runMatch2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/sweep/index.js
var tooSmall, sweep, sweep_default;
var init_sweep = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/sweep/index.js"() {
    init_getHooks();
    init_trim_down();
    init_runMatch();
    tooSmall = function(maybeList, document) {
    ...
    sweep = function(document, net3, methods17, opts2 = {}) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/tagger/canBe.js
var canBe, canBe_default;
var init_canBe = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/tagger/canBe.js"() {
    canBe = function(terms, tag, model5) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/tagger/index.js
var tagger, tagger_default;
var init_tagger = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/tagger/index.js"() {
    init_canBe();
    tagger = function(list4, document, world2) {
      const { model: model5, methods: methods17 } = world2;
  ...
  \x1B[32m\u2192 ${list4.length} post-tagger:\x1B[0m`);
    ...
    tagger_default = tagger;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/index.js
var methods_default6;
var init_methods7 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/methods/index.js"() {
    init_buildNet();
    init_sweep();
    init_tagger();
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/sweep/plugin.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/methods/setTag.js
var isMulti, addChunk, tagTerm, multiTag, isArray9, log, setTag, setTag_default;
var init_setTag = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/methods/setTag.js"() {
    isMulti = / /;
    addChunk = function(term, tag) {
      if (tag === "Noun") {
        term.chunk = tag;
      }
      if (tag === "Verb") {
    ...
    tagTerm = function(term, tag, tagSet, isSafe) {
    ...
    multiTag = function(terms, tagString2, tagSet, isSafe) {
    ...
    setTag = function(terms, tag, world2 = {}, isSafe, reason) {
    ...
    setTag_default = setTag;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/methods/unTag.js
var unTag, unTag_default;
var init_unTag = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/methods/unTag.js"() {
    unTag = function(terms, tag, tagSet) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/methods/canBe.js
var canBe2, canBe_default2;
var init_canBe2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/methods/canBe.js"() {
    canBe2 = function(term, tag, tagSet) {
      if (!tagSet.hasOwnProperty(tag)) {
        return true;
      }
      const not = tagSet[tag].not || [];
    ...
    canBe_default2 = canBe2;
  }
});

// node_modules/.pnpm/grad-school@0.0.5/node_modules/grad-school/builds/grad-school.mjs
var e, t2, n, r, i2, c, s, h, o, l, a, p, d, u, f, j, g, _;
var init_grad_school = __esm({
  "node_modules/.pnpm/grad-school@0.0.5/node_modules/grad-school/builds/grad-school.mjs"() {
    e = function(e2) {
    ...
    i2 = (e2) => "[object Array]" === Object.prototype.toString.call(e2);
    c = (e2) => (e2 = e2 || "").trim();
    s = function(c2 = []) {
    ...
    h = (e2) => "\x1B[31m" + e2 + "\x1B[0m";
    o = (e2) => "\x1B[2m" + e2 + "\x1B[0m";
    l = function(e2, t3) {
      let n3 = "-> ";
      t3 && (n3 = o("\u2192 "));
      let i3 = "";
    ...
    f = (e2, t3) => (Object.keys(t3).forEach(((n3) => {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/methods/addTags/_colors.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/methods/addTags/02-fmt.js
var getColor, fmt, fmt_default;
var init_fmt = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/methods/addTags/02-fmt.js"() {
    init_colors();
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/methods/addTags/01-validate.js
var toArr, addImplied, validate, validate_default;
var init_validate = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/methods/addTags/01-validate.js"() {
    toArr = function(input) {
      if (!input) {
        return [];
      }
      if (typeof input === "string") {
    ...
    addImplied = function(tags, already) {
    ...
    validate = function(tags, already) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/methods/addTags/index.js
var compute3, fromUser, addTags, addTags_default;
var init_addTags = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/methods/addTags/index.js"() {
    init_grad_school();
    init_fmt();
    init_validate();
    compute3 = function(allTags2) {
    ...
    addTags_default = addTags;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/methods/index.js
var methods_default7;
var init_methods8 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/methods/index.js"() {
    init_setTag();
    init_unTag();
    init_canBe2();
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/api/tag.js
var isArray10, fns5, tag_default;
var init_tag = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/api/tag.js"() {
    isArray10 = function(arr) {
      return Object.prototype.toString.call(arr) === "[object Array]";
    };
    fns5 = {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/api/index.js
var tagAPI, api_default8;
var init_api8 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/api/index.js"() {
    init_tag();
    tagAPI = function(View2) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/lib.js
var addTags2, lib_default4;
var init_lib8 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/lib.js"() {
    addTags2 = function(tags) {
    ...
    lib_default4 = { addTags: addTags2 };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/compute/tagRank.js
var boringTags, sortByKids, tagRank, tagRank_default;
var init_tagRank = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/compute/tagRank.js"() {
    boringTags = /* @__PURE__ */ new Set(["Auxiliary", "Possessive"]);
    sortByKids = function(tags, tagSet) {
      tags = tags.sort((a2, b) => {
    ...
    tagRank_default = tagRank;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tag/plugin.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/01-sentences/01-simple-split.js
var initSplit, splitsOnly, newLine, basicSplit, simple_split_default;
var init_simple_split = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/01-sentences/01-simple-split.js"() {
    initSplit = /([.!?\u203D\u2E18\u203C\u2047-\u2049\u3002]+\s)/g;
    splitsOnly = /^[.!?\u203D\u2E18\u203C\u2047-\u2049\u3002]+\s$/;
    newLine = /((?:\r?\n|\r)+)/;
    basicSplit = function(text) {
      const all4 = [];
    ...
    simple_split_default = basicSplit;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/01-sentences/02-simple-merge.js
var hasLetter, hasSomething, notEmpty, simple_merge_default;
var init_simple_merge = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/01-sentences/02-simple-merge.js"() {
    hasLetter = /[a-z0-9\u00C0-\u00FF\u00a9\u00ae\u2000-\u3300\ud000-\udfff]/i;
    hasSomething = /\S/;
    notEmpty = function(splits) {
      const chunks2 = [];
    ...
    simple_merge_default = notEmpty;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/01-sentences/03-smart-merge.js
var hasNewline, smartMerge, smart_merge_default;
var init_smart_merge = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/01-sentences/03-smart-merge.js"() {
    hasNewline = function(c2) {
      return Boolean(c2.match(/\n$/));
    };
    smartMerge = function(chunks2, world2) {
    ...
    smart_merge_default = smartMerge;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/01-sentences/04-quote-merge.js
var MAX_QUOTE, pairs, openQuote, closeQuote, closesQuote, quoteMerge, quote_merge_default;
var init_quote_merge = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/01-sentences/04-quote-merge.js"() {
    MAX_QUOTE = 280;
    pairs = {
      '"': '"',
    ...
    openQuote = RegExp("[" + Object.keys(pairs).join("") + "]", "g");
    closeQuote = RegExp("[" + Object.values(pairs).join("") + "]", "g");
    closesQuote = function(str) {
      if (!str) {
        return false;
      }
    ...
    quote_merge_default = quoteMerge;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/01-sentences/05-parens-merge.js
var MAX_LEN, hasOpen, hasClosed, mergeParens, parens_merge_default;
var init_parens_merge = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/01-sentences/05-parens-merge.js"() {
    MAX_LEN = 250;
    hasOpen = /\(/g;
    hasClosed = /\)/g;
    mergeParens = function(splits) {
      const arr = [];
      for (let i3 = 0; i3 < splits.length; i3 += 1) {
        const split3 = splits[i3];
    ...
    parens_merge_default = mergeParens;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/01-sentences/index.js
var hasSomething2, startWhitespace, splitSentences, sentences_default;
var init_sentences = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/01-sentences/index.js"() {
    ...
    hasSomething2 = /\S/;
    startWhitespace = /^\s+/;
    splitSentences = function(text, world2) {
      text = text || "";
      text = String(text);
    ...
    sentences_default = splitSentences;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/02-terms/01-hyphens.js
var hasHyphen2, splitHyphens2;
var init_hyphens = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/02-terms/01-hyphens.js"() {
    hasHyphen2 = function(str, model5) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/02-terms/03-ranges.js
var combineRanges, ranges_default;
var init_ranges = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/02-terms/03-ranges.js"() {
    combineRanges = function(arr) {
    ...
    ranges_default = combineRanges;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/02-terms/02-slashes.js
var isSlash, combineSlashes, slashes_default;
var init_slashes = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/02-terms/02-slashes.js"() {
    isSlash = new RegExp("\\p{L} ?\\/ ?\\p{L}+$", "u");
    combineSlashes = function(arr) {
    ...
    slashes_default = combineSlashes;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/02-terms/index.js
var wordlike, isBoundary, naiiveSplit, notWord, isArray11, splitWords, terms_default;
var init_terms = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/02-terms/index.js"() {
    ...
    isArray11 = function(arr) {
      return Object.prototype.toString.call(arr) === "[object Array]";
    };
    splitWords = function(str, model5) {
      let result = [];
      let arr = [];
      str = str || "";
    ...
    terms_default = splitWords;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/03-whitespace/tokenize.js
var isLetter, isNumber, hasAcronym, chillin, normalizePunctuation, tokenize_default;
var init_tokenize = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/03-whitespace/tokenize.js"() {
    isLetter = new RegExp("\\p{Letter}", "u");
    isNumber = /[\p{Number}\p{Currency_Symbol}]/u;
    hasAcronym = /^[a-z]\.([a-z]\.)+/i;
    chillin = /[sn]['’]$/;
    normalizePunctuation = function(str, model5) {
    ...
    tokenize_default = normalizePunctuation;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/03-whitespace/index.js
var parseTerm, whitespace_default2;
var init_whitespace2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/03-whitespace/index.js"() {
    init_tokenize();
    parseTerm = (txt, model5) => {
    ...
    whitespace_default2 = parseTerm;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/unicode.js
var killUnicode, unicode_default;
var init_unicode = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/unicode.js"() {
    killUnicode = function(str, world2) {
    ...
    unicode_default = killUnicode;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/normal/01-cleanup.js
var clean, cleanup_default;
var init_cleanup = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/normal/01-cleanup.js"() {
    ...
    cleanup_default = clean;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/normal/02-acronyms.js
var periodAcronym, oneLetterAcronym, noPeriodAcronym, lowerCaseAcronym, isAcronym, doAcronym, acronyms_default;
var init_acronyms = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/normal/02-acronyms.js"() {
    periodAcronym = /([A-Z]\.)+[A-Z]?,?$/;
    oneLetterAcronym = /^[A-Z]\.,?$/;
    noPeriodAcronym = /[A-Z]{2,}('s|,)?$/;
    lowerCaseAcronym = /([a-z]\.)+[a-z]\.?$/;
    isAcronym = function(str) {
    ...
    doAcronym = function(str) {
      if (isAcronym(str)) {
        str = str.replace(/\./g, "");
      }
      return str;
    };
    acronyms_default = doAcronym;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/normal/index.js
var normalize, normal_default;
var init_normal = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/normal/index.js"() {
    init_cleanup();
    init_acronyms();
    normalize = function(term, world2) {
      const killUnicode2 = world2.methods.one.killUnicode;
      let str = term.text || "";
    ...
    normal_default = normalize;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/parse.js
var parse2, parse_default2;
var init_parse2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/parse.js"() {
    init_normal();
    parse2 = function(input, world2) {
      const { methods: methods17, model: model5 } = world2;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/01-sentences/is-sentence.js
var isAcronym2, hasEllipse, hasLetter2, hasPeriod, leadInit, isSentence, is_sentence_default;
var init_is_sentence = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/01-sentences/is-sentence.js"() {
    isAcronym2 = /[ .][A-Z]\.? *$/i;
    hasEllipse = /(?:\u2026|\.{2,}) *$/;
    hasLetter2 = new RegExp("\\p{L}", "u");
    hasPeriod = /\. *$/;
    leadInit = /^[A-Z]\. $/;
    isSentence = function(str, abbrevs) {
    ...
    is_sentence_default = isSentence;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/index.js
var methods_default8;
var init_methods9 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/methods/index.js"() {
    init_sentences();
    init_terms();
    init_whitespace2();
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/aliases.js
...
var init_aliases = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/aliases.js"() {
    ...
    aliases_default = aliases;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/abbreviations/misc.js
var misc_default;
var init_misc = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/abbreviations/misc.js"() {
    misc_default = [
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/abbreviations/honorifics.js
var honorifics_default;
var init_honorifics = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/abbreviations/honorifics.js"() {
    honorifics_default = [
      "adj",
      "adm",
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/abbreviations/months.js
var months_default;
var init_months = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/abbreviations/months.js"() {
    months_default = ["jan", "feb", "mar", "apr", "jun", "jul", "aug", "sep", "sept", "oct", "nov", "dec"];
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/abbreviations/nouns.js
var nouns_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/abbreviations/organizations.js
var organizations_default;
var init_organizations = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/abbreviations/organizations.js"() {
    organizations_default = ["dept", "univ", "assn", "bros", "inc", "ltd", "co"];
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/abbreviations/places.js
var places_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/abbreviations/units.js
var units_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/lexicon.js
var list, abbreviations, lexicon2;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/prefixes.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/suffixes.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/unicode.js
var compact, unicode, unicode_default2;
var init_unicode2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/unicode.js"() {
    compact = {
      "!": "\xA1",
      "?": "\xBF\u0241",
    ...
    unicode_default2 = unicode;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/punctuation.js
var prePunctuation, postPunctuation, emoticons;
var init_punctuation = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/punctuation.js"() {
    prePunctuation = {
      "#": true,
      //#hastag
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/model/index.js
var model_default2;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/alias.js
var hasSlash, hasDomain, isMath, addAliases, alias_default;
var init_alias = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/alias.js"() {
    hasSlash = /\//;
    hasDomain = /[a-z]\.[a-z]/i;
    isMath = /[0-9]/;
    addAliases = function(term, world2) {
    ...
    alias_default = addAliases;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/machine.js
var hasDash3, doMachine, machine_default;
var init_machine = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/machine.js"() {
    hasDash3 = new RegExp("^\\p{Letter}+-\\p{Letter}+$", "u");
    doMachine = function(term) {
    ...
    machine_default = doMachine;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/freq.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/offset.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/reindex.js
var index2, reindex_default;
var init_reindex = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/reindex.js"() {
    index2 = function(view) {
    ...
    reindex_default = index2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/wordCount.js
var wordCount2, wordCount_default;
var init_wordCount = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/wordCount.js"() {
    wordCount2 = function(view) {
      let n3 = 0;
      const docs = view.docs;
      for (let i3 = 0; i3 < docs.length; i3 += 1) {
        for (let t3 = 0; t3 < docs[i3].length; t3 += 1) {
    ...
    wordCount_default = wordCount2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/index.js
var termLoop, methods16, compute_default7;
var init_compute7 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/compute/index.js"() {
    init_alias();
    init_normal();
    init_machine();
    ...
    compute_default7 = methods16;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/tokenize/plugin.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/typeahead/compute.js
var typeahead, compute_default8;
var init_compute8 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/typeahead/compute.js"() {
    typeahead = function(view) {
    ...
    compute_default8 = { typeahead };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/typeahead/api.js
var autoFill, api2, api_default9;
var init_api9 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/typeahead/api.js"() {
    autoFill = function() {
      const docs = this.docs;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/typeahead/lib/allPrefixes.js
var getPrefixes, allPrefixes_default;
var init_allPrefixes = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/typeahead/lib/allPrefixes.js"() {
    getPrefixes = function(arr, opts2, world2) {
      let index3 = {};
      const collisions = [];
    ...
    allPrefixes_default = getPrefixes;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/typeahead/lib/index.js
var isObject7, defaults2, prepare, lib_default5;
var init_lib9 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/typeahead/lib/index.js"() {
    init_allPrefixes();
    isObject7 = (val) => {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/1-one/typeahead/plugin.js
var model3, plugin_default13;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/one.js
var one_default;
var init_one = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/one.js"() {
    init_nlp();
    ...
    one_default = nlp_default;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/irregulars/plurals.js
var plurals_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/lexicon/_data.js
var data_default;
var init_data = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/lexicon/_data.js"() {
    data_default = {
...
// node_modules/.pnpm/efrt@2.7.0/node_modules/efrt/src/encoding.js
var BASE, seq, cache, toAlphaCode, fromAlphaCode, encoding_default;
var init_encoding = __esm({
  "node_modules/.pnpm/efrt@2.7.0/node_modules/efrt/src/encoding.js"() {
    BASE = 36;
    seq = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    cache = seq.split("").reduce(function(h2, c2, i3) {
      h2[c2] = i3;
      return h2;
    }, {});
    toAlphaCode = function(n3) {
      if (seq[n3] !== void 0) {
        return seq[n3];
      }
      let places2 = 1;
...
// node_modules/.pnpm/efrt@2.7.0/node_modules/efrt/src/unpack/symbols.js
...
// node_modules/.pnpm/efrt@2.7.0/node_modules/efrt/src/unpack/traverse.js
var indexFromRef, toArray, unpack, traverse_default;
var init_traverse = __esm({
  "node_modules/.pnpm/efrt@2.7.0/node_modules/efrt/src/unpack/traverse.js"() {
    init_symbols();
    init_encoding();
    indexFromRef = function(trie, ref, index3) {
    ...
    traverse_default = unpack;
  }
});

// node_modules/.pnpm/efrt@2.7.0/node_modules/efrt/src/unpack/index.js
var unpack2, unpack_default;
var init_unpack = __esm({
  "node_modules/.pnpm/efrt@2.7.0/node_modules/efrt/src/unpack/index.js"() {
    init_traverse();
    unpack2 = function(str) {
      if (!str) {
        return {};
      }
    ...
    unpack_default = unpack2;
  }
});

// node_modules/.pnpm/efrt@2.7.0/node_modules/efrt/src/index.js
var init_src = __esm({
  "node_modules/.pnpm/efrt@2.7.0/node_modules/efrt/src/index.js"() {
    init_unpack();
  }
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/lexicon/misc.js
var prp, misc, misc_default2;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/lexicon/frozenLex.js
var frozenLex_default;
var init_frozenLex = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/lexicon/frozenLex.js"() {
    frozenLex_default = {
      "20th century fox": "Organization",
      "7 eleven": "Organization",
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/lexicon/emoticons.js
var emoticons_default;
var init_emoticons = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/lexicon/emoticons.js"() {
    emoticons_default = [
      ":(",
      ":)",
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/nouns/toPlural/_rules.js
var suffixes, rules_default;
var init_rules = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/nouns/toPlural/_rules.js"() {
    ...
    rules_default = suffixes;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/nouns/toPlural/index.js
var addE, trySuffix, pluralize, toPlural_default;
var init_toPlural = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/nouns/toPlural/index.js"() {
    init_rules();
    addE = /([xsz]|ch|sh)$/;
    trySuffix = function(str) {
    ...
    toPlural_default = pluralize;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/lexicon/index.js
var hasSwitch, lexicon3, switches, tmpModel;
var init_lexicon2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/lexicon/index.js"() {
    ...
    init_plurals();
    hasSwitch = /\|/;
    lexicon3 = misc_default2;
    switches = {};
    tmpModel = { two: { irregularPlurals: plurals_default, uncountable: {} } };
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/_noun.js
var n2, noun_default;
var init_noun = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/_noun.js"() {
    n2 = "Singular";
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/_verb.js
var v, verb_default;
var init_verb = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/_verb.js"() {
    v = "Infinitive";
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/actor-verb.js
var clue, actor_verb_default;
var init_actor_verb = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/actor-verb.js"() {
    init_noun();
    init_verb();
    clue = {
    ...
    actor_verb_default = clue;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/_adj.js
var jj, adj_default;
var init_adj = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/_adj.js"() {
    jj = "Adjective";
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/_gerund.js
var g2, gerund_default;
var init_gerund = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/_gerund.js"() {
    g2 = "Gerund";
    gerund_default = {
      beforeTags: {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/adj-gerund.js
var g3, jj2, clue2, adj_gerund_default;
var init_adj_gerund = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/adj-gerund.js"() {
    init_adj();
    init_gerund();
    g3 = "Gerund";
    ...
    adj_gerund_default = clue2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/adj-noun.js
var misc2, clue3, adj_noun_default;
var init_adj_noun = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/adj-noun.js"() {
    init_adj();
    init_noun();
    misc2 = {
    ...
    adj_noun_default = clue3;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/adj-past.js
var past, jj3, adjPast, adj_past_default;
var init_adj_past = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/adj-past.js"() {
    init_adj();
    past = "PastTense";
    jj3 = "Adjective";
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/adj-present.js
var misc3, clue4, adj_present_default;
var init_adj_present = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/adj-present.js"() {
    init_adj();
    init_verb();
    misc3 = {
    ...
    adj_present_default = clue4;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/noun-gerund.js
var misc4, clue5, noun_gerund_default;
var init_noun_gerund = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/noun-gerund.js"() {
    init_noun();
    init_gerund();
    misc4 = {
    ...
    noun_gerund_default = clue5;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/noun-verb.js
var nn, vb, clue6, noun_verb_default;
var init_noun_verb = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/noun-verb.js"() {
    init_noun();
    init_verb();
    nn = "Singular";
    ...
    noun_verb_default = clue6;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/_person.js
var p2, person_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/person-date.js
var m, p3, month, person_date_default;
var init_person_date = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/person-date.js"() {
    init_person();
    m = "Month";
    p3 = "Person";
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/person-noun.js
var clue7, person_noun_default;
var init_person_noun = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/person-noun.js"() {
    init_person();
    init_noun();
    clue7 = {
    ...
    person_noun_default = clue7;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/person-verb.js
var clues, person_verb_default;
var init_person_verb = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/person-verb.js"() {
    init_person();
    init_verb();
    init_noun();
    ...
    person_verb_default = clues;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/person-place.js
var p4, place, clue8, person_place_default;
var init_person_place = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/person-place.js"() {
    init_person();
    p4 = "Place";
    place = {
    ...
    person_place_default = clue8;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/person-adj.js
var clues2, person_adj_default;
var init_person_adj = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/person-adj.js"() {
    init_person();
    init_adj();
    clues2 = {
    ...
    person_adj_default = clues2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/unit-noun.js
var un, clues3, unit_noun_default;
var init_unit_noun = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/unit-noun.js"() {
    un = "Unit";
    clues3 = {
      beforeTags: { Value: un },
    ...
    unit_noun_default = clues3;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/index.js
var clues4, copy, clues_default;
var init_clues = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/clues/index.js"() {
    init_actor_verb();
    init_adj_gerund();
    init_adj_noun();
    ...
    clues_default = clues4;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/patterns/suffixes.js
var Adj, Inf, Pres, Sing, Past, Avb, Plrl, Actor, Vb, Noun, Prop, Last, Modal, Place, Prt, suffixes_default2;
var init_suffixes2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/patterns/suffixes.js"() {
    Adj = "Adjective";
    Inf = "Infinitive";
    Pres = "PresentTense";
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/patterns/prefixes.js
var Adj2, Noun2, Verb, prefixes_default2;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/patterns/endsWith.js
var Adj3, Inf2, Pres2, Sing2, Past2, Adverb, Exp, Actor2, Verb2, Noun3, Last2, endsWith_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/patterns/neighbours.js
var vb2, nn2, neighbours_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/models/_data.js
var data_default2;
...
// node_modules/.pnpm/suffix-thumb@5.0.2/node_modules/suffix-thumb/src/convert/index.js
var checkEx, checkSame, checkRules, convert, convert_default;
var init_convert = __esm({
  "node_modules/.pnpm/suffix-thumb@5.0.2/node_modules/suffix-thumb/src/convert/index.js"() {
    checkEx = function(str, ex = {}) {
      if (ex.hasOwnProperty(str)) {
        return ex[str];
      }
      return null;
    };
    checkSame = function(str, same = []) {
    ...
    checkRules = function(str, fwd, both = {}) {
      fwd = fwd || {};
...
// node_modules/.pnpm/suffix-thumb@5.0.2/node_modules/suffix-thumb/src/reverse/index.js
var flipObj, reverse2, reverse_default;
var init_reverse = __esm({
  "node_modules/.pnpm/suffix-thumb@5.0.2/node_modules/suffix-thumb/src/reverse/index.js"() {
    ...
    reverse_default = reverse2;
  }
});

// node_modules/.pnpm/suffix-thumb@5.0.2/node_modules/suffix-thumb/src/compress/unpack.js
var prefix2, toObject, growObject, unpackOne, uncompress, unpack_default2;
var init_unpack2 = __esm({
  "node_modules/.pnpm/suffix-thumb@5.0.2/node_modules/suffix-thumb/src/compress/unpack.js"() {
    prefix2 = /^([0-9]+)/;
    toObject = function(txt) {
      let obj = {};
    ...
    unpack_default2 = uncompress;
  }
});

// node_modules/.pnpm/suffix-thumb@5.0.2/node_modules/suffix-thumb/src/index.js
var init_src2 = __esm({
  "node_modules/.pnpm/suffix-thumb@5.0.2/node_modules/suffix-thumb/src/index.js"() {
    init_convert();
    init_reverse();
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/models/index.js
var fromPast, fromPresent, fromGerund, fromParticiple, toPast, toPresent, toGerund, toParticiple, toComparative, toSuperlative, fromComparative, fromSuperlative, adjToNoun, models_default;
var init_models = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/models/index.js"() {
    init_data2();
    init_src2();
    fromPast = unpack_default2(data_default2.PastTense);
    fromPresent = unpack_default2(data_default2.PresentTense);
    fromGerund = unpack_default2(data_default2.Gerund);
    fromParticiple = unpack_default2(data_default2.Participle);
    toPast = reverse_default(fromPast);
    toPresent = reverse_default(fromPresent);
    toGerund = reverse_default(fromGerund);
    toParticiple = reverse_default(fromParticiple);
    toComparative = unpack_default2(data_default2.Comparative);
    toSuperlative = unpack_default2(data_default2.Superlative);
    fromComparative = reverse_default(toComparative);
    fromSuperlative = reverse_default(toSuperlative);
    adjToNoun = unpack_default2(data_default2.AdjToNoun);
    models_default = {
      fromPast,
      fromPresent,
      fromGerund,
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/regex/regex-normal.js
var regex_normal_default;
var init_regex_normal = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/regex/regex-normal.js"() {
    regex_normal_default = [
      //web tags
      [/^[\w.]+@[\w.]+\.[a-z]{2,3}$/, "Email"],
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/regex/regex-text.js
var regex_text_default;
var init_regex_text = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/regex/regex-text.js"() {
    regex_text_default = [
      // #coolguy
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/regex/regex-numbers.js
var regex_numbers_default;
var init_regex_numbers = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/regex/regex-numbers.js"() {
    regex_numbers_default = [
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/orgWords.js
var orgWords_default;
var init_orgWords = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/orgWords.js"() {
    orgWords_default = [
      "academy",
      "administration",
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/placeWords.js
var placeWords_default;
var init_placeWords = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/placeWords.js"() {
    placeWords_default = [
      // geology
      "atoll",
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/nouns/toSingular/_rules.js
var rules_default2;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/nouns/toSingular/index.js
var invertObj, toSingular, toSingular_default;
var init_toSingular = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/nouns/toSingular/index.js"() {
    init_rules2();
    ...
    toSingular = function(str, model5) {
    ...
    toSingular_default = toSingular;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/nouns/index.js
var all, nouns_default2;
var init_nouns2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/nouns/index.js"() {
    init_toPlural();
    init_toSingular();
    all = function(str, model5) {
      const arr = [str];
    ...
    nouns_default2 = { toPlural: toPlural_default, toSingular: toSingular_default, all };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/verbs/getTense/_guess.js
var guessVerb, guess_default;
var init_guess = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/verbs/getTense/_guess.js"() {
    ...
    guess_default = guessVerb;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/verbs/getTense/index.js
var getTense, getTense_default;
var init_getTense = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/verbs/getTense/index.js"() {
    init_guess();
    getTense = function(str) {
    ...
    getTense_default = getTense;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/verbs/toInfinitive/index.js
var toParts, copulaMap, toInfinitive, toInfinitive_default;
var init_toInfinitive = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/verbs/toInfinitive/index.js"() {
    init_src2();
    init_getTense();
    toParts = function(str, model5) {
      let prefix5 = "";
      let prefixes2 = {};
    ...
    toInfinitive = function(str, model5, tense) {
    ...
    toInfinitive_default = toInfinitive;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/verbs/conjugate/index.js
var parse3, conjugate, conjugate_default;
var init_conjugate = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/verbs/conjugate/index.js"() {
    ...
    conjugate_default = conjugate;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/verbs/index.js
var all2, verbs_default;
var init_verbs = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/verbs/index.js"() {
    init_toInfinitive();
    init_conjugate();
    all2 = function(str, model5) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/adjectives/inflect.js
var toSuperlative2, toComparative2, fromComparative2, fromSuperlative2, toNoun;
var init_inflect = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/adjectives/inflect.js"() {
    init_src2();
    toSuperlative2 = function(adj, model5) {
      const mod = model5.two.models.toSuperlative;
      return convert_default(adj, mod);
    };
    toComparative2 = function(adj, model5) {
      const mod = model5.two.models.toComparative;
      return convert_default(adj, mod);
    };
    fromComparative2 = function(adj, model5) {
      const mod = model5.two.models.fromComparative;
      return convert_default(adj, mod);
    };
    fromSuperlative2 = function(adj, model5) {
      const mod = model5.two.models.fromSuperlative;
      return convert_default(adj, mod);
    };
    toNoun = function(adj, model5) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/adjectives/conjugate/lib.js
var suffixLoop, lib_default6;
var init_lib10 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/adjectives/conjugate/lib.js"() {
    suffixLoop = function(str = "", suffixes5 = []) {
    ...
    lib_default6 = suffixLoop;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/adjectives/conjugate/fromAdverb.js
var s2, ical, suffixes2, noAdj, exceptions, toAdjective, fromAdverb_default;
var init_fromAdverb = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/adjectives/conjugate/fromAdverb.js"() {
    init_lib10();
    s2 = "ically";
    ical = /* @__PURE__ */ new Set([
      "analyt" + s2,
      //analytical
      "chem" + s2,
      // chemical
    ...
    fromAdverb_default = toAdjective;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/adjectives/conjugate/toAdverb.js
var suffixes3, exceptions2, toAdverb, toAdverb_default;
var init_toAdverb = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/adjectives/conjugate/toAdverb.js"() {
    init_lib10();
    suffixes3 = [
      null,
    ...
    toAdverb_default = toAdverb;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/adjectives/index.js
var all3, adjectives_default;
var init_adjectives = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/adjectives/index.js"() {
    init_inflect();
    init_fromAdverb();
    init_toAdverb();
    all3 = function(str, model5) {
      let arr = [str];
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/transform/index.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/expand/byTag.js
var byTag_default;
var init_byTag = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/expand/byTag.js"() {
    byTag_default = {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/expand/index.js
var expand3, expand_default2;
var init_expand2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/expand/index.js"() {
    init_byTag();
    expand3 = function(words, world2) {
      const { methods: methods17, model: model5 } = world2;
      const lex = {};
      const _multi = {};
      Object.keys(words).forEach((word) => {
    ...
    expand_default2 = expand3;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/quickSplit.js
var splitOn, quickSplit, quickSplit_default;
var init_quickSplit = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/quickSplit.js"() {
    splitOn = function(terms, i3) {
      const isNum = /^[0-9]+$/;
      const term = terms[i3];
      if (!term) {
        return false;
    ...
    quickSplit_default = quickSplit;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/looksPlural.js
var isPlural, exceptions3, notPlural, looksPlural, looksPlural_default;
var init_looksPlural = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/looksPlural.js"() {
    isPlural = {
    ...
    looksPlural_default = looksPlural;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/index.js
var methods_default9;
var init_methods10 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/methods/index.js"() {
    init_transform();
    init_expand2();
    init_quickSplit();
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/_expand/irregulars.js
var expandIrregulars, irregulars_default;
var init_irregulars = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/_expand/irregulars.js"() {
    expandIrregulars = function(model5) {
      const { irregularPlurals } = model5.two;
      const { lexicon: lexicon4 } = model5.one;
    ...
    irregulars_default = expandIrregulars;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/_expand/index.js
var tmpModel2, switchDefaults, expandLexicon, addUncountables, expandVerb, expandAdjective, expandNoun, expandVariable, expand4, expand_default3;
var init_expand3 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/_expand/index.js"() {
    init_methods10();
    init_irregulars();
    init_toPlural();
    ...
    expandLexicon = function(words, model5) {
    ...
    addUncountables = function(words, model5) {
      Object.keys(words).forEach((k2) => {
    ...
    expandVerb = function(str, words, doPresent) {
    ...
    expandAdjective = function(str, words, model5) {
    ...
    expandNoun = function(str, words, model5) {
      const plur = toPlural_default(str, model5);
      words[plur] = words[plur] || "Plural";
    };
    expandVariable = function(switchWords, model5) {
      const words = {};
      const lex = model5.one.lexicon;
    ...
    expand_default3 = expand4;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/index.js
var model4, model_default3;
var init_model3 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/model/index.js"() {
    ...
    model_default3 = model4;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/1st-pass/01-colons.js
var byPunctuation, colons_default;
var init_colons = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/1st-pass/01-colons.js"() {
    byPunctuation = function(terms, i3, model5, world2) {
    ...
    colons_default = byPunctuation;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/1st-pass/02-hyphens.js
var byHyphen, hyphens_default;
var init_hyphens2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/1st-pass/02-hyphens.js"() {
    byHyphen = function(terms, i3, model5, world2) {
      const setTag2 = world2.methods.one.setTag;
      if (terms[i3].post === "-" && terms[i3 + 1]) {
        setTag2([terms[i3], terms[i3 + 1]], "Hyphenated", world2, null, `1-punct-hyphen''`);
      }
    };
    hyphens_default = byHyphen;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/2nd-pass/00-tagSwitch.js
var prefix3, tagSwitch, tagSwitch_default;
var init_tagSwitch = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/2nd-pass/00-tagSwitch.js"() {
    prefix3 = /^(under|over|mis|re|un|dis|semi)-?/;
    tagSwitch = function(terms, i3, model5) {
    ...
    tagSwitch_default = tagSwitch;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/_fastTag.js
var log2, fastTag, fastTag_default;
var init_fastTag = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/_fastTag.js"() {
    ...
    fastTag = function(term, tag, reason) {
      if (!tag || tag.length === 0) {
        return;
      }
      if (term.frozen === true) {
    ...
    fastTag_default = fastTag;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/_fillTags.js
var uncountable, setPluralSingular, setTense, fillTags, fillTags_default;
var init_fillTags = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/_fillTags.js"() {
    init_fastTag();
    init_looksPlural();
    init_getTense();
    ...
    setPluralSingular = function(term) {
    ...
    fillTags = function(terms, i3, model5) {
      const term = terms[i3];
      const tags = Array.from(term.tags);
    ...
    fillTags_default = fillTags;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/2nd-pass/01-case.js
var titleCase2, hasNumber, notProper, hasIVX, romanNumeral, romanNumValid, nope, checkCase, case_default2;
var init_case2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/2nd-pass/01-case.js"() {
    init_fastTag();
    init_fillTags();
    titleCase2 = new RegExp("^\\p{Lu}[\\p{Ll}'\u2019]", "u");
    hasNumber = /[0-9]/;
    notProper = ["Date", "Month", "WeekDay", "Unit", "Expression"];
    hasIVX = /[IVX]/;
    romanNumeral = /^[IVXLCDM]{2,}$/;
    romanNumValid = /^M{0,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})$/;
    nope = {
      li: true,
      dc: true,
      md: true,
    ...
    checkCase = function(terms, i3, model5) {
      const term = terms[i3];
    ...
    case_default2 = checkCase;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/2nd-pass/02-suffix.js
var suffixLoop2, tagBySuffix, suffix_default;
var init_suffix = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/2nd-pass/02-suffix.js"() {
    init_fastTag();
    suffixLoop2 = function(str = "", suffixes5 = []) {
      const len = str.length;
      let max3 = 7;
      if (len <= max3) {
        max3 = len - 1;
    ...
    tagBySuffix = function(terms, i3, model5) {
      const term = terms[i3];
      if (term.tags.size === 0) {
    ...
    suffix_default = tagBySuffix;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/2nd-pass/03-regex.js
var hasApostrophe, doRegs, doEndsWith, checkRegex, regex_default;
var init_regex = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/2nd-pass/03-regex.js"() {
    hasApostrophe = /['‘’‛‵′`´]/;
    doRegs = function(str, regs) {
      for (let i3 = 0; i3 < regs.length; i3 += 1) {
    ...
    doEndsWith = function(str = "", byEnd3) {
    ...
    checkRegex = function(terms, i3, model5, world2) {
      const setTag2 = world2.methods.one.setTag;
    ...
    regex_default = checkRegex;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/2nd-pass/04-prefix.js
var prefixLoop, checkPrefix, prefix_default;
var init_prefix = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/2nd-pass/04-prefix.js"() {
    init_fastTag();
    prefixLoop = function(str = "", prefixes2 = []) {
      const len = str.length;
      let max3 = 7;
      if (max3 > len - 3) {
        max3 = len - 3;
    ...
    checkPrefix = function(terms, i3, model5) {
      const term = terms[i3];
      if (term.tags.size === 0) {
    ...
    prefix_default = checkPrefix;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/2nd-pass/05-year.js
var min, max2, dateWords, seemsGood, seemsOkay, seemsFine, tagYear, year_default;
var init_year = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/2nd-pass/05-year.js"() {
    init_fastTag();
    min = 1400;
    max2 = 2100;
    dateWords = /* @__PURE__ */ new Set([
      "in",
      "on",
      "by",
      "until",
    ...
    year_default = tagYear;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/07-verb-type.js
var verbType, verb_type_default;
var init_verb_type = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/07-verb-type.js"() {
    verbType = function(terms, i3, model5, world2) {
      const setTag2 = world2.methods.one.setTag;
      const term = terms[i3];
    ...
    verb_type_default = verbType;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/01-acronym.js
var oneLetterAcronym2, isUpperCase, upperThenS, periodAcronym2, noPeriodAcronym2, lowerCaseAcronym2, oneLetterWord, places, isNoPeriodAcronym, isAcronym3, acronym_default;
var init_acronym = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/01-acronym.js"() {
    init_fastTag();
    oneLetterAcronym2 = /^[A-Z]('s|,)?$/;
    isUpperCase = /^[A-Z-]+$/;
    upperThenS = /^[A-Z]+s$/;
    periodAcronym2 = /([A-Z]\.)+[A-Z]?,?$/;
    noPeriodAcronym2 = /[A-Z]{2,}('s|,)?$/;
    lowerCaseAcronym2 = /([a-z]\.)+[a-z]\.?$/;
    oneLetterWord = {
      I: true,
      A: true
    };
    ...
    isNoPeriodAcronym = function(term, model5) {
      let str = term.text;
    ...
    isAcronym3 = function(terms, i3, model5) {
      const term = terms[i3];
    ...
    acronym_default = isAcronym3;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/02-neighbours.js
var lookAtWord, lookAtTag, neighbours, neighbours_default2;
var init_neighbours2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/02-neighbours.js"() {
    init_fastTag();
    init_fillTags();
    lookAtWord = function(term, words) {
      if (!term) {
        return null;
      }
    ...
    neighbours = function(terms, i3, model5) {
    ...
    neighbours_default2 = neighbours;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/03-orgWords.js
var isTitleCase3, isOrg, tagOrgs, orgWords_default2;
var init_orgWords2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/03-orgWords.js"() {
    isTitleCase3 = (str) => new RegExp("^\\p{Lu}[\\p{Ll}'\u2019]", "u").test(str);
    isOrg = function(term, i3, yelling) {
      if (!term) {
        return false;
      }
    ...
    tagOrgs = function(terms, i3, world2, yelling) {
    ...
    orgWords_default2 = tagOrgs;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/04-placeWords.js
var isTitleCase4, isPossessive, placeCont, noBefore, isPlace, tagOrgs2, placeWords_default2;
var init_placeWords2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/04-placeWords.js"() {
    isTitleCase4 = (str) => new RegExp("^\\p{Lu}[\\p{Ll}'\u2019]", "u").test(str);
    isPossessive = /'s$/;
    placeCont = /* @__PURE__ */ new Set([
      "athletic",
      "city",
      "community",
      "eastern",
    ...
    noBefore = /* @__PURE__ */ new Set(["center", "centre", "way", "range", "bar", "bridge", "field", "pit"]);
    isPlace = function(term, i3, yelling) {
      if (!term) {
        return false;
      }
      const tags = term.tags;
    ...
    tagOrgs2 = function(terms, i3, world2, yelling) {
    ...
    placeWords_default2 = tagOrgs2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/05-fallback.js
var nounFallback, fallback_default;
var init_fallback = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/05-fallback.js"() {
    init_fastTag();
    init_fillTags();
    nounFallback = function(terms, i3, model5) {
      let isEmpty = false;
    ...
    fallback_default = nounFallback;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/_adhoc.js
var isTitleCase5, isCapital, isAlone, isEndNoun, isStart, adhoc, adhoc_default;
var init_adhoc = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/_adhoc.js"() {
    isTitleCase5 = /^[A-Z][a-z]/;
    isCapital = (terms, i3) => {
    ...
    isAlone = (terms, i3, tag) => {
      if (i3 === 0 && !terms[1]) {
        return tag;
      }
      return null;
    };
    isEndNoun = function(terms, i3) {
      if (!terms[i3 + 1] && terms[i3 - 1] && terms[i3 - 1].tags.has("Determiner")) {
        return "Noun";
      }
      return null;
    };
    isStart = function(terms, i3, tag) {
      if (i3 === 0 && terms.length > 3) {
        return tag;
      }
      return null;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/06-switches.js
var env, prefix4, checkWord, checkTag, pickTag, doSwitches, switches_default;
var init_switches = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/06-switches.js"() {
    init_fillTags();
    init_adhoc();
    env = typeof process === "undefined" || !process.env ? self.env || {} : process.env;
    prefix4 = /^(under|over|mis|re|un|dis|semi)-?/;
    checkWord = (term, obj) => {
      if (!term || !obj) {
        return null;
      }
  ...
  \x1B[2m\x1B[3m     \u2193 - '${str}' \x1B[0m`);
    ...
    pickTag = function(terms, i3, clues5, model5) {
      if (!clues5) {
        return null;
      }
    ...
    doSwitches = function(terms, i3, world2) {
 ...
 -> X  - '${str}'  : (${form})  `);
    ...
    switches_default = doSwitches;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/08-imperative.js
var beside, imperative, imperative_default;
var init_imperative = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/3rd-pass/08-imperative.js"() {
    ...
    imperative = function(terms, world2) {
      const setTag2 = world2.methods.one.setTag;
    ...
    imperative_default = imperative;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/index.js
var ignoreCase, firstPass, secondPass, thirdPass, preTagger, tagger_default2;
var init_tagger2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/tagger/index.js"() {
    init_colons();
    init_hyphens2();
    init_tagSwitch();
    ...
    firstPass = function(docs, model5, world2) {
      docs.forEach((terms) => {
        colons_default(terms, 0, model5, world2);
      });
    };
    secondPass = function(terms, model5, world2, isYelling) {
      for (let i3 = 0; i3 < terms.length; i3 += 1) {
    ...
    thirdPass = function(terms, model5, world2, isYelling) {
      for (let i3 = 0; i3 < terms.length; i3 += 1) {
    ...
    tagger_default2 = preTagger;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/root.js
var toRoot, getRoot, root_default;
var init_root = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/root.js"() {
    toRoot = {
      // 'spencer's' -> 'spencer'
    ...
    root_default = getRoot;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/penn.js
var mapping, toPenn, pennTag, penn_default;
var init_penn = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/penn.js"() {
    mapping = {
      // adverbs
    ...
    penn_default = pennTag;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/index.js
var compute_default9;
var init_compute9 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/compute/index.js"() {
    init_tagger2();
    init_root();
    init_penn();
    compute_default9 = { preTagger: tagger_default2, root: root_default, penn: penn_default };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/tagSet/nouns.js
var entity, nouns_default3;
var init_nouns3 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/tagSet/nouns.js"() {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/tagSet/verbs.js
var verbs_default2;
var init_verbs2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/tagSet/verbs.js"() {
    verbs_default2 = {
      Verb: {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/tagSet/values.js
var values_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/tagSet/dates.js
var dates_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/tagSet/misc.js
var anything, misc_default3;
var init_misc3 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/tagSet/misc.js"() {
    anything = ["Noun", "Verb", "Adjective", "Adverb", "Value", "QuestionWord"];
    misc_default3 = {
      Adjective: {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/tagSet/index.js
var allTags, tagSet_default;
var init_tagSet = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/tagSet/index.js"() {
    init_nouns3();
    init_verbs2();
    init_values();
    init_dates();
    init_misc3();
    allTags = Object.assign({}, nouns_default3, verbs_default2, values_default, dates_default, misc_default3);
    tagSet_default = allTags;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/preTagger/plugin.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/api/contract.js
var postPunct, setContraction, contract, contract_default;
var init_contract = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/api/contract.js"() {
    postPunct = /[,)"';:\-–—.…]/;
    setContraction = function(m3, suffix) {
      if (!m3.found) {
        return;
      }
    ...
    contract_default = contract;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/api/index.js
var titleCase3, toTitleCase3, api3, api_default10;
var init_api10 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/api/index.js"() {
    init_contract();
    titleCase3 = new RegExp("^\\p{Lu}[\\p{Ll}'\u2019]", "u");
    toTitleCase3 = function(str = "") {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/compute/_splice.js
var insertContraction2, splice_default2;
var init_splice2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/compute/_splice.js"() {
    insertContraction2 = function(document, point, words) {
      const [n3, w] = point;
      if (!words || words.length === 0) {
        return;
      }
    ...
    splice_default2 = insertContraction2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/compute/apostrophe-s.js
var hasContraction3, hasWords, isWords, adjLike, isOrHas, apostropheS, apostrophe_s_default;
var init_apostrophe_s = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/compute/apostrophe-s.js"() {
    hasContraction3 = /'/;
    hasWords = /* @__PURE__ */ new Set([
      "been",
      //the meeting's been ..
      "become"
      //my son's become
    ]);
    isWords = /* @__PURE__ */ new Set([
      "what",
      //it's what
      "how",
      //it's how
    ...
    adjLike = /* @__PURE__ */ new Set(["too", "also", "enough"]);
    isOrHas = (terms, i3) => {
    ...
    apostrophe_s_default = apostropheS;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/compute/apostrophe-d.js
var hasContraction4, hadWords, wouldWords, hadOrWould, _apostropheD2, apostrophe_d_default2;
var init_apostrophe_d2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/compute/apostrophe-d.js"() {
    hasContraction4 = /'/;
    hadWords = /* @__PURE__ */ new Set([
      "better",
      //had better
      "done",
      //had done
    ...
    _apostropheD2 = function(terms, i3) {
    ...
    apostrophe_d_default2 = _apostropheD2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/compute/apostrophe-t.js
var lastNoun, apostropheT2, apostrophe_t_default2;
var init_apostrophe_t2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/compute/apostrophe-t.js"() {
    lastNoun = function(terms, i3) {
      for (let n3 = i3 - 1; n3 >= 0; n3 -= 1) {
    ...
    apostropheT2 = function(terms, i3) {
      if (terms[i3].normal === "ain't" || terms[i3].normal === "aint") {
    ...
    apostrophe_t_default2 = apostropheT2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/compute/isPossessive.js
var banList, beforePossessive, adjLike2, nounLike, isPossessive2, isPossessive_default;
var init_isPossessive = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/compute/isPossessive.js"() {
    banList = {
      that: true,
      there: true,
    ...
    adjLike2 = /* @__PURE__ */ new Set(["too", "also", "enough", "about"]);
    nounLike = /* @__PURE__ */ new Set(["is", "are", "did", "were", "could", "should", "must", "had", "have"]);
    isPossessive2 = (terms, i3) => {
      const term = terms[i3];
    ...
    isPossessive_default = isPossessive2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/compute/index.js
var byApostrophe2, reIndex, reTag2, byEnd2, toDocs2, contractionTwo, compute_default10;
var init_compute10 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/compute/index.js"() {
    init_splice2();
    init_apostrophe_s();
    init_apostrophe_d2();
    ...
    reTag2 = function(terms, view, start2, len) {
      const tmp = view.update();
      tmp.document = [terms];
      let end2 = start2 + len;
      if (start2 > 0) {
    ...
    compute_default10 = { contractionTwo };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/plugin.js
var plugin_default15;
var init_plugin15 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/contraction-two/plugin.js"() {
    init_api10();
    init_compute10();
    plugin_default15 = {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/adjective/adjective.js
var adjective_default;
var init_adjective = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/adjective/adjective.js"() {
    adjective_default = [
      // all fell apart
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/adjective/adj-adverb.js
var adverbAdj, noLy, adj_adverb_default;
var init_adj_adverb = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/adjective/adj-adverb.js"() {
    adverbAdj = `(dark|bright|flat|light|soft|pale|dead|dim|faux|little|wee|sheer|most|near|good|extra|all)`;
    noLy = "(hard|fast|late|early|high|right|deep|close|direct)";
    adj_adverb_default = [
      // kinda sparkly
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/adjective/adj-gerund.js
var adj_gerund_default2;
var init_adj_gerund2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/adjective/adj-gerund.js"() {
    adj_gerund_default2 = [
      //a staggering cost
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/adjective/adj-noun.js
var adj_noun_default2;
var init_adj_noun2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/adjective/adj-noun.js"() {
    adj_noun_default2 = [
      //the above is clear
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/adjective/adj-verb.js
var adj_verb_default;
var init_adj_verb = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/adjective/adj-verb.js"() {
    adj_verb_default = [
      // amusing his aunt
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/adverb.js
var adverb_default;
var init_adverb = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/adverb.js"() {
    adverb_default = [
      //still good
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/dates/date-phrase.js
var date_phrase_default;
var init_date_phrase = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/dates/date-phrase.js"() {
    date_phrase_default = [
      // ==== Holiday ====
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/dates/date.js
var date_default;
var init_date = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/dates/date.js"() {
    date_default = [
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/nouns/nouns.js
var infNouns, nouns_default4;
var init_nouns4 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/nouns/nouns.js"() {
    infNouns = "(feel|sense|process|rush|side|bomb|bully|challenge|cover|crush|dump|exchange|flow|function|issue|lecture|limit|march|process)";
    nouns_default4 = [
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/verbs/noun-gerund.js
var noun_gerund_default2;
var init_noun_gerund2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/verbs/noun-gerund.js"() {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/verbs/verb-noun.js
var verb_noun_default;
var init_verb_noun = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/verbs/verb-noun.js"() {
    verb_noun_default = [
      // do the dance
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/numbers/money.js
var money_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/numbers/fractions.js
var fractions_default;
var init_fractions = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/numbers/fractions.js"() {
    fractions_default = [
      // half a penny
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/numbers/numbers.js
var numbers_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/person/person-phrase.js
var person_phrase_default;
var init_person_phrase = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/person/person-phrase.js"() {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/person/ambig-name.js
var ambig_name_default;
var init_ambig_name = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/person/ambig-name.js"() {
    ambig_name_default = [
      // ebenezer scrooge
      {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/verbs/verbs.js
var verbs_default3;
var init_verbs3 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/verbs/verbs.js"() {
    verbs_default3 = [
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/verbs/auxiliary.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/verbs/phrasal.js
var phrasal_default;
var init_phrasal = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/verbs/phrasal.js"() {
    phrasal_default = [
      // ==== Phrasal ====
      //'foo-up'
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/verbs/imperative.js
var notIf2, imperative_default2;
var init_imperative2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/verbs/imperative.js"() {
    notIf2 = "(i|we|they)";
    imperative_default2 = [
      // do not go
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/verbs/adj-gerund.js
var adj_gerund_default3;
var init_adj_gerund3 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/verbs/adj-gerund.js"() {
    adj_gerund_default3 = [
      // that were growing
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/verbs/passive.js
var passive_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/_misc.js
var matches, misc_default4;
var init_misc4 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/_misc.js"() {
    matches = [
      // u r cool
    ...
    misc_default4 = matches;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/nouns/organizations.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/nouns/places.js
var places_default2;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/conjunctions.js
var conjunctions_default;
var init_conjunctions = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/conjunctions.js"() {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/expressions.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/model/index.js
var matches2, model_default4;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/compute/index.js
var net, postTagger, tagger2, compute_default11;
var init_compute11 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/compute/index.js"() {
    net = null;
    postTagger = function(view) {
    ...
    tagger2 = (view) => view.compute(["freeze", "lexicon", "preTagger", "postTagger", "unfreeze"]);
    compute_default11 = { postTagger, tagger: tagger2 };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/api.js
...
var round;
var init_api11 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/api.js"() {
    round = (n3) => Math.round(n3 * 100) / 100;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/plugin.js
var plugin2, plugin_default16;
var init_plugin16 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/postTagger/plugin.js"() {
    init_model4();
    init_compute11();
    init_api11();
    ...
    plugin_default16 = plugin2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/lazy/maybeMatch.js
var getWords, maybeMatch, maybeMatch_default;
var init_maybeMatch = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/lazy/maybeMatch.js"() {
    getWords = function(net3) {
      return Object.keys(net3.hooks).filter((w) => !w.startsWith("#") && !w.startsWith("%"));
    };
    maybeMatch = function(doc, net3) {
    ...
    maybeMatch_default = maybeMatch;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/lazy/lazyParse.js
var lazyParse, lazyParse_default;
var init_lazyParse = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/lazy/lazyParse.js"() {
    init_maybeMatch();
    lazyParse = function(input, reg) {
      let net3 = reg;
    ...
    lazyParse_default = lazyParse;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/lazy/plugin.js
var plugin_default17;
var init_plugin17 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/lazy/plugin.js"() {
    init_lazyParse();
    plugin_default17 = {
      lib: {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/swap/api/swap-verb.js
var matchVerb, swapVerb, swap_verb_default;
var init_swap_verb = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/swap/api/swap-verb.js"() {
    matchVerb = function(m3, lemma) {
    ...
    swap_verb_default = swapVerb;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/swap/api/swap.js
var swapNoun, swapAdverb, swapAdjective, swap, swap_default;
var init_swap = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/swap/api/swap.js"() {
    init_swap_verb();
    swapNoun = function(m3, lemma) {
      let str = lemma;
      if (m3.has("#Plural")) {
    ...
    swapAdjective = function(m3, lemma) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/swap/plugin.js
var api4, plugin_default18;
var init_plugin18 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/2-two/swap/plugin.js"() {
    init_swap();
    api4 = function(View2) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/two.js
var two_default;
var init_two = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/two.js"() {
    init_one();
    ...
    two_default = one_default;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/adjectives/plugin.js
var toRoot2, api5, plugin_default19;
var init_plugin19 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/adjectives/plugin.js"() {
    toRoot2 = function(adj) {
    ...
    plugin_default19 = { api: api5 };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/adverbs/plugin.js
var toRoot3, api6, plugin_default20;
var init_plugin20 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/adverbs/plugin.js"() {
    toRoot3 = function(adj) {
    ...
    plugin_default20 = { api: api6 };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/api/clauses.js
var byComma, splitParentheses, splitQuotes, clauses, clauses_default;
var init_clauses = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/api/clauses.js"() {
    byComma = function(doc) {
    ...
    clauses_default = clauses;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/api/chunks.js
var chunks, chunks_default2;
var init_chunks2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/api/chunks.js"() {
    ...
    chunks_default2 = chunks;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/api/api.js
var api7, api_default12;
var init_api12 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/api/api.js"() {
    init_clauses();
    init_chunks2();
    api7 = function(View2) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/compute/01-easy.js
var byWord2, easyMode, easy_default;
var init_easy = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/compute/01-easy.js"() {
    byWord2 = {
      this: "Noun",
    ...
    easy_default = easyMode;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/compute/02-neighbours.js
var byNeighbour, neighbours_default3;
var init_neighbours3 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/compute/02-neighbours.js"() {
    byNeighbour = function(document) {
      for (let n3 = 0; n3 < document.length; n3 += 1) {
        for (let t3 = 0; t3 < document[n3].length; t3 += 1) {
          const term = document[n3][t3];
          if (term.chunk) {
    ...
    neighbours_default3 = byNeighbour;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/compute/03-matcher.js
var rules, net2, matcher, matcher_default;
var init_matcher = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/compute/03-matcher.js"() {
    ...
    net2 = null;
    matcher = function(view, _2, world2) {
      const { methods: methods17 } = world2;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/compute/04-fallback.js
var setChunk, fallback, fallback_default2;
var init_fallback2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/compute/04-fallback.js"() {
    setChunk = function(term, chunk) {
      const env2 = typeof process === "undefined" || !process.env ? self.env || {} : process.env;
    ...
    fallback_default2 = fallback;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/compute/05-fixUp.js
var fixUp, fixUp_default;
var init_fixUp = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/compute/05-fixUp.js"() {
    fixUp = function(docs) {
      const byChunk = [];
      let current = null;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/compute/index.js
var findChunks, compute_default12;
var init_compute12 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/compute/index.js"() {
    init_easy();
    init_neighbours3();
    init_matcher();
    ...
    compute_default12 = { chunks: findChunks };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/plugin.js
var plugin_default21;
var init_plugin21 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/chunker/plugin.js"() {
    init_api12();
    init_compute12();
    plugin_default21 = {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/acronyms/index.js
var hasPeriod2, api8, acronyms_default2;
var init_acronyms2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/acronyms/index.js"() {
    hasPeriod2 = /\./g;
    api8 = function(View2) {
    ...
    acronyms_default2 = api8;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/parentheses/fns.js
var hasOpen2, hasClosed2, findEnd, find2, strip;
var init_fns = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/parentheses/fns.js"() {
    hasOpen2 = /\(/;
    hasClosed2 = /\)/;
    findEnd = function(terms, i3) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/parentheses/index.js
var api9, parentheses_default;
var init_parentheses = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/parentheses/index.js"() {
    ...
    parentheses_default = api9;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/possessives/index.js
var apostropheS2, find3, api10, possessives_default;
var init_possessives = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/possessives/index.js"() {
    apostropheS2 = /'s$/;
    find3 = function(doc) {
    ...
    possessives_default = api10;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/quotations/fns.js
var pairs2, hasOpen3, hasClosed3, findEnd2, find4, strip2;
var init_fns2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/quotations/fns.js"() {
    pairs2 = {
      '"': '"',
    ...
    hasOpen3 = RegExp("[" + Object.keys(pairs2).join("") + "]");
    hasClosed3 = RegExp("[" + Object.values(pairs2).join("") + "]");
    findEnd2 = function(terms, i3) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/quotations/index.js
var api11, quotations_default;
var init_quotations = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/quotations/index.js"() {
    init_fns2();
    api11 = function(View2) {
    ...
    quotations_default = api11;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/selections/index.js
var phoneNumbers, selections, aliases2, addMethods, selections_default;
var init_selections = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/selections/index.js"() {
    phoneNumbers = function(n3) {
    ...
    selections_default = addMethods;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/slashes/index.js
var hasSlash2, api12, slashes_default2;
var init_slashes2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/slashes/index.js"() {
    hasSlash2 = /\//;
    api12 = function(View2) {
    ...
    slashes_default2 = api12;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/plugin.js
var plugin_default22;
var init_plugin22 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/misc/plugin.js"() {
    init_acronyms2();
    init_parentheses();
    init_possessives();
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/normalize/methods.js
var termLoop2, methods_default10;
var init_methods11 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/normalize/methods.js"() {
    termLoop2 = function(view, cb) {
      view.docs.forEach((terms) => {
        terms.forEach(cb);
      });
    };
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/normalize/api.js
...
var split2, light, medium, heavy, presets;
var init_api13 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/normalize/api.js"() {
    init_methods11();
    split2 = (str) => {
    ...
    light = "unicode|punctuation|whitespace|acronyms";
    medium = "|case|contractions|parentheses|quotations|emoji|honorifics|debullet";
    heavy = "|possessives|adverbs|nouns|verbs";
    presets = {
      light: split2(light),
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/normalize/plugin.js
var plugin_default23;
var init_plugin23 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/normalize/plugin.js"() {
    init_api13();
    plugin_default23 = {
      api: api_default13
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/find.js
var findNouns, find_default;
var init_find = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/find.js"() {
    findNouns = function(doc) {
    ...
    find_default = findNouns;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/api/isSubordinate.js
var list2, isSubordinate, isSubordinate_default;
var init_isSubordinate = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/api/isSubordinate.js"() {
    list2 = [
      "after",
      "although",
    ...
    isSubordinate_default = isSubordinate;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/api/isPlural.js
var notPlural2, isPlural2, isPlural_default;
var init_isPlural = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/api/isPlural.js"() {
    notPlural2 = "(#Pronoun|#Place|#Value|#Person|#Uncountable|#Month|#WeekDay|#Holiday|#Possessive)";
    isPlural2 = function(m3, root) {
      if (m3.has("#Plural")) {
        return true;
      }
    ...
    isPlural_default = isPlural2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/api/parse.js
var getRoot2, parseNoun, parse_default3;
var init_parse3 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/api/parse.js"() {
    init_isSubordinate();
    init_isPlural();
    getRoot2 = function(m3) {
      let tmp = m3.clone();
      tmp = tmp.match("#Noun+");
    ...
    parse_default3 = parseNoun;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/api/toJSON.js
var toText2, toArray2, getNum, toJSON2, toJSON_default;
var init_toJSON = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/api/toJSON.js"() {
    init_parse3();
    toText2 = (m3) => m3.text();
    toArray2 = (m3) => m3.json({ terms: false, normal: true }).map((s3) => s3.normal);
    getNum = function(m3) {
      const num = null;
      if (!m3.found) {
        return num;
    ...
    toJSON_default = toJSON2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/api/hasPlural.js
var hasPlural, hasPlural_default;
var init_hasPlural = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/api/hasPlural.js"() {
    hasPlural = function(root) {
      if (root.has("^(#Uncountable|#ProperNoun|#Place|#Pronoun|#Acronym)+$")) {
        return false;
      }
      return true;
    };
    hasPlural_default = hasPlural;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/api/toPlural.js
var keep, nounToPlural, toPlural_default2;
var init_toPlural2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/api/toPlural.js"() {
    init_hasPlural();
    keep = { tags: true };
    nounToPlural = function(m3, parsed) {
    ...
    toPlural_default2 = nounToPlural;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/api/toSingular.js
var keep2, nounToSingular, toSingular_default2;
var init_toSingular2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/api/toSingular.js"() {
    keep2 = { tags: true };
    nounToSingular = function(m3, parsed) {
    ...
    toSingular_default2 = nounToSingular;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/api/api.js
var api13, api_default14;
var init_api14 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/api/api.js"() {
    init_find();
    init_parse3();
    init_toJSON();
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/plugin.js
var plugin_default24;
var init_plugin24 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/nouns/plugin.js"() {
    init_api14();
    plugin_default24 = {
      api: api_default14
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/fractions/find.js
var findFractions, find_default2;
var init_find2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/fractions/find.js"() {
    findFractions = function(doc, n3) {
    ...
    find_default2 = findFractions;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/parse/toNumber/findModifiers.js
var findModifiers, findModifiers_default;
var init_findModifiers = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/parse/toNumber/findModifiers.js"() {
    findModifiers = (str) => {
      const mults = [
        {
    ...
    findModifiers_default = findModifiers;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/parse/toNumber/data.js
var data_default3;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/parse/toNumber/validate.js
var isValid, validate_default2;
var init_validate2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/parse/toNumber/validate.js"() {
    ...
    validate_default2 = isValid;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/parse/toNumber/parseDecimals.js
var parseDecimals, parseDecimals_default;
var init_parseDecimals = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/parse/toNumber/parseDecimals.js"() {
    init_data3();
    parseDecimals = function(arr) {
      let str = "0.";
      for (let i3 = 0; i3 < arr.length; i3++) {
        const w = arr[i3];
    ...
    parseDecimals_default = parseDecimals;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/parse/toNumber/parseNumeric.js
var parseNumeric, parseNumeric_default;
var init_parseNumeric = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/parse/toNumber/parseNumeric.js"() {
    parseNumeric = (str) => {
    ...
    parseNumeric_default = parseNumeric;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/parse/toNumber/index.js
var improperFraction, casualForms, section_sum, parse4, toNumber_default;
var init_toNumber = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/parse/toNumber/index.js"() {
    ...
    toNumber_default = parse4;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/fractions/parse.js
var endS, parseNumber, mapping2, slashForm, nOutOfN, nOrinalth, oneNth, named, round2, parseFraction, parse_default4;
var init_parse4 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/fractions/parse.js"() {
    init_toNumber();
    endS = /s$/;
    parseNumber = function(m3) {
    ...
    parse_default4 = parseFraction;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/_toString.js
var numToString, toString_default;
var init_toString = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/_toString.js"() {
    numToString = function(n3) {
      if (n3 < 1e6) {
        return String(n3);
      }
      let str;
    ...
    toString_default = numToString;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/format/toText/data.js
var tens_mapping, ones_mapping, sequence;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/format/toText/index.js
var breakdown_magnitudes, breakdown_hundred, handle_decimal, toText3, toText_default;
var init_toText = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/format/toText/index.js"() {
    init_toString();
    init_data4();
    breakdown_magnitudes = function(num) {
      let working = num;
      const have = [];
      sequence.forEach((a2) => {
        if (num >= a2[0]) {
    ...
    breakdown_hundred = function(num) {
      const arr = [];
      if (num > 100) {
        return arr;
      }
    ...
    toText_default = toText3;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/fractions/convert/toCardinal.js
var toCardinal, toCardinal_default;
var init_toCardinal = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/fractions/convert/toCardinal.js"() {
    init_toText();
    toCardinal = function(obj) {
    ...
    toCardinal_default = toCardinal;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/format/toOrdinal/textOrdinal.js
var irregulars, textOrdinal, textOrdinal_default;
var init_textOrdinal = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/format/toOrdinal/textOrdinal.js"() {
    init_toText();
    irregulars = {
      one: "first",
    ...
    textOrdinal_default = textOrdinal;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/fractions/convert/toOrdinal.js
var toOrdinal, toOrdinal_default;
var init_toOrdinal = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/fractions/convert/toOrdinal.js"() {
    init_toText();
    init_textOrdinal();
    toOrdinal = function(obj) {
      if (!obj.numerator || !obj.denominator) {
        return "";
      }
    ...
    toOrdinal_default = toOrdinal;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/fractions/api.js
var plugin3, api_default15;
var init_api15 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/fractions/api.js"() {
    init_find2();
    init_parse4();
    init_toCardinal();
    init_toOrdinal();
    plugin3 = function(View2) {
    ...
    api_default15 = plugin3;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/find.js
var ones, tens, teens, findNumbers, find_default3;
var init_find3 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/find.js"() {
    ones = "one|two|three|four|five|six|seven|eight|nine";
    tens = "twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety|fourty";
    teens = "eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen";
    findNumbers = function(doc) {
    ...
    find_default3 = findNumbers;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/parse/index.js
var parseNumeric2, parseNumber2, parse_default5;
var init_parse5 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/parse/index.js"() {
    init_toNumber();
    init_parse4();
    parseNumeric2 = function(str, m3) {
      str = str.replace(/,/g, "");
    ...
    parse_default5 = parseNumber2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/format/toOrdinal/numOrdinal.js
var numOrdinal, numOrdinal_default;
var init_numOrdinal = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/format/toOrdinal/numOrdinal.js"() {
    init_toString();
    numOrdinal = function(obj) {
      const num = obj.num;
      if (!num && num !== 0) {
        return null;
      }
    ...
    numOrdinal_default = numOrdinal;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/format/suffix.js
var prefixes, suffixes4, addSuffix, suffix_default2;
var init_suffix2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/format/suffix.js"() {
    prefixes = {
      "\xA2": "cents",
      $: "dollars",
    ...
    suffix_default2 = addSuffix;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/format/index.js
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/isUnit.js
var isArray12, coerceToObject, isUnit, isUnit_default;
var init_isUnit = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/isUnit.js"() {
    init_parse5();
    isArray12 = (arr) => Object.prototype.toString.call(arr) === "[object Array]";
    coerceToObject = function(input) {
    ...
    isUnit_default = isUnit;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/api.js
var addMethod, api_default16;
var init_api16 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/numbers/api.js"() {
    init_find3();
    init_parse5();
    init_format();
    init_isUnit();
    addMethod = function(View2) {
    ...
    api_default16 = addMethod;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/plugin.js
var api14, plugin_default25;
var init_plugin25 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/numbers/plugin.js"() {
    init_api15();
    init_api16();
    api14 = function(View2) {
      api_default15(View2);
      api_default16(View2);
    };
    plugin_default25 = {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/redact/plugin.js
var defaults3, redact, plugin4, plugin_default26;
var init_plugin26 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/redact/plugin.js"() {
    defaults3 = {
      people: true,
      emails: true,
    ...
    plugin_default26 = plugin4;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/questions.js
var isQuestion, findQuestions, questions_default;
var init_questions = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/questions.js"() {
    isQuestion = function(doc) {
    ...
    questions_default = findQuestions;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/parse/mainClause.js
var subordinate, relative, mainClause, mainClause_default;
var init_mainClause = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/parse/mainClause.js"() {
    subordinate = `(after|although|as|because|before|if|since|than|that|though|when|whenever|where|whereas|wherever|whether|while|why|unless|until|once)`;
    relative = `(that|which|whichever|who|whoever|whom|whose|whomever)`;
    mainClause = function(s3) {
      let m3 = s3;
      if (m3.length === 1) {
        return m3;
      }
    ...
    mainClause_default = mainClause;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/parse/index.js
var grammar, parse5, parse_default6;
var init_parse6 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/parse/index.js"() {
    init_mainClause();
    grammar = function(vb3) {
      let tense = null;
    ...
    parse_default6 = parse5;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/conjugate/toPast.js
var toPast2, toPast_default;
var init_toPast = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/conjugate/toPast.js"() {
    toPast2 = function(s3) {
      let verbs = s3.verbs();
    ...
    toPast_default = toPast2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/conjugate/toPresent.js
var toPresent2, toPresent_default;
var init_toPresent = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/conjugate/toPresent.js"() {
    toPresent2 = function(s3) {
      let verbs = s3.verbs();
      const first = verbs.eq(0);
    ...
    toPresent_default = toPresent2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/conjugate/toFuture.js
var toFuture, toFuture_default;
var init_toFuture = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/conjugate/toFuture.js"() {
    toFuture = function(s3) {
      let verbs = s3.verbs();
      const first = verbs.eq(0);
      first.toFutureTense();
      s3 = s3.fullSentence();
    ...
    toFuture_default = toFuture;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/conjugate/toNegative.js
var toNegative, toPositive;
var init_toNegative = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/conjugate/toNegative.js"() {
    toNegative = function(s3) {
      s3.verbs().first().toNegative().compute("chunks");
      return s3;
    };
    toPositive = function(s3) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/conjugate/toInfinitive.js
var toInfinitive2, toInfinitive_default2;
var init_toInfinitive2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/conjugate/toInfinitive.js"() {
    toInfinitive2 = function(s3) {
      s3.verbs().toInfinitive();
      return s3;
    };
    toInfinitive_default2 = toInfinitive2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/api.js
var api15, api_default17;
var init_api17 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/api.js"() {
    init_questions();
    init_parse6();
    init_toPast();
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/plugin.js
var plugin_default27;
var init_plugin27 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/sentences/plugin.js"() {
    init_api17();
    plugin_default27 = { api: api_default17 };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/people/find.js
var find5, find_default4;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/people/parse.js
var parse6, parse_default7;
var init_parse7 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/people/parse.js"() {
    ...
    parse_default7 = parse6;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/people/gender.js
var m2, f2, honorifics, predictGender, gender_default;
var init_gender = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/people/gender.js"() {
    ...
    predictGender = function(parsed, person) {
    ...
    gender_default = predictGender;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/people/api.js
var addMethod2, api_default18;
var init_api18 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/people/api.js"() {
    init_find4();
    init_parse7();
    init_gender();
    addMethod2 = function(View2) {
    ...
    api_default18 = addMethod2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/places/find.js
var find6, find_default5;
var init_find5 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/places/find.js"() {
    find6 = function(doc) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/places/api.js
var addMethod3, api_default19;
var init_api19 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/places/api.js"() {
    init_find5();
    addMethod3 = function(View2) {
    ...
    api_default19 = addMethod3;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/orgs/api.js
var api16, api_default20;
var init_api20 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/orgs/api.js"() {
    api16 = function(View2) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/topics.js
var find7, api17, topics_default;
var init_topics = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/topics.js"() {
    ...
    api17 = function(View2) {
      View2.prototype.topics = find7;
    };
    topics_default = api17;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/plugin.js
var api18, plugin_default28;
var init_plugin28 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/topics/plugin.js"() {
    init_api18();
    init_api19();
    init_api20();
    init_topics();
    api18 = function(View2) {
      api_default18(View2);
      api_default19(View2);
      api_default20(View2);
      topics_default(View2);
    };
    plugin_default28 = { api: api18 };
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/find.js
var findVerbs, find_default6;
var init_find6 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/find.js"() {
    findVerbs = function(doc) {
    ...
    find_default6 = findVerbs;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/parse/root.js
var getMain, root_default2;
var init_root2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/parse/root.js"() {
    getMain = function(vb3) {
      let root = vb3;
      if (vb3.wordCount() > 1) {
    ...
    root_default2 = getMain;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/parse/adverbs.js
var getAdverbs, adverbs_default;
var init_adverbs = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/parse/adverbs.js"() {
    getAdverbs = function(vb3, root) {
      const res = {
        pre: vb3.none(),
        post: vb3.none()
      };
    ...
    adverbs_default = getAdverbs;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/parse/index.js
var getAuxiliary, getNegative, getPhrasal, parseVerb, parse_default8;
var init_parse8 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/parse/index.js"() {
    init_root2();
    init_adverbs();
    getAuxiliary = function(vb3, root) {
    ...
    getNegative = function(vb3) {
      return vb3.match("#Negative");
    };
    getPhrasal = function(root) {
    ...
    parse_default8 = parseVerb;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/parse/grammar/forms.js
var present, conditional, future, prog, past2, complete, passive, plural, singular, getData, verbForms, list3, forms_default;
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/parse/grammar/index.js
var cleanUp2, isInfinitive, getGrammar, grammar_default;
var init_grammar = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/parse/grammar/index.js"() {
    init_forms();
    cleanUp2 = function(vb3, res) {
      vb3 = vb3.clone();
    ...
    grammar_default = getGrammar;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/parse/getSubject.js
var shouldSkip, noSubClause, lastNoun2, isPlural3, getSubject, getSubject_default;
var init_getSubject = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/parse/getSubject.js"() {
    shouldSkip = function(last) {
      if (last.length <= 1) {
        return false;
      }
    ...
    getSubject_default = getSubject;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/lib.js
var noop, isPlural4, wasWere, isAreAm, doDoes, getTense2, toInf, noWill;
var init_lib11 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/lib.js"() {
    init_getSubject();
    noop = (vb3) => vb3;
    isPlural4 = (vb3, parsed) => {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/toJSON.js
var toArray3, toText4, toInf2, toJSON3, toJSON_default2;
var init_toJSON2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/toJSON.js"() {
    init_parse8();
    init_grammar();
    init_lib11();
    toArray3 = function(m3) {
      if (!m3 || !m3.isView) {
        return [];
      }
    ...
    toJSON_default2 = toJSON3;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/conjugate/toInfinitive.js
var keep3, toInf3, toInfinitive_default3;
var init_toInfinitive3 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/conjugate/toInfinitive.js"() {
    init_lib11();
    keep3 = { tags: true };
    toInf3 = function(vb3, parsed) {
      const { toInfinitive: toInfinitive3 } = vb3.methods.two.transform.verb;
    ...
    toInfinitive_default3 = toInf3;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/conjugate/toPast.js
var keep4, fns6, forms, toPast3, toPast_default2;
var init_toPast2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/conjugate/toPast.js"() {
    init_lib11();
    keep4 = { tags: true };
    fns6 = {
    ...
    toPast3 = function(vb3, parsed, form) {
    ...
    toPast_default2 = toPast3;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/conjugate/toParticiple.js
var haveHas, simple, forms2, toPast4, toParticiple_default;
var init_toParticiple = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/conjugate/toParticiple.js"() {
    init_lib11();
    haveHas = function(vb3, parsed) {
      const subj = getSubject_default(vb3, parsed);
      const m3 = subj.subject;
      if (m3.has("(i|we|you)")) {
        return "have";
    ...
    toPast4 = function(vb3, parsed, form) {
    ...
    toParticiple_default = toPast4;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/conjugate/toPresent.js
var keep5, simple2, toGerund2, vbToInf, forms3, toPresent3, toPresent_default2;
var init_toPresent2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/conjugate/toPresent.js"() {
    init_lib11();
    keep5 = { tags: true };
    simple2 = (vb3, parsed) => {
    ...
    toPresent3 = function(vb3, parsed, form) {
    ...
    toPresent_default2 = toPresent3;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/conjugate/toFuture.js
var keep6, simple3, progressive, forms4, toFuture2, toFuture_default2;
var init_toFuture2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/conjugate/toFuture.js"() {
    init_lib11();
    keep6 = { tags: true };
    simple3 = (vb3, parsed) => {
    ...
    toFuture2 = function(vb3, parsed, form) {
    ...
    toFuture_default2 = toFuture2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/conjugate/toGerund.js
var keep7, toGerund3, toGerund_default;
var init_toGerund = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/conjugate/toGerund.js"() {
    init_lib11();
    keep7 = { tags: true };
    toGerund3 = function(vb3, parsed) {
    ...
    toGerund_default = toGerund3;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/conjugate/toNegative.js
var keep8, doesNot, isWas, hasCopula, forms5, toNegative2, toNegative_default;
var init_toNegative2 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/conjugate/toNegative.js"() {
    init_lib11();
    keep8 = { tags: true };
    doesNot = function(vb3, parsed) {
      const does = doDoes(vb3, parsed);
    ...
    hasCopula = (vb3) => vb3.has("(is|was|am|are|will|were|be)");
    forms5 = {
    ...
    toNegative2 = function(vb3, parsed, form) {
      if (vb3.has("#Negative")) {
        return vb3;
      }
    ...
    toNegative_default = toNegative2;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/api.js
var api19, api_default21;
var init_api21 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/api/api.js"() {
    init_find6();
    init_toJSON2();
    init_parse8();
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/plugin.js
var plugin_default29;
var init_plugin29 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/verbs/plugin.js"() {
    init_api21();
    plugin_default29 = {
      api: api_default21
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/coreference/compute/lib.js
var findChained, prevSentence;
var init_lib12 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/coreference/compute/lib.js"() {
    findChained = function(want, s3) {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/coreference/compute/findPerson.js
var byGender, getPerson, findPerson_default;
var init_findPerson = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/coreference/compute/findPerson.js"() {
    init_lib12();
    byGender = function(ppl, gender) {
      if (gender === "m") {
    ...
    findPerson_default = getPerson;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/coreference/compute/findThey.js
var getThey, findThey_default;
var init_findThey = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/coreference/compute/findThey.js"() {
    init_lib12();
    getThey = function(s3) {
    ...
    findThey_default = getThey;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/coreference/compute/index.js
var addReference, stepBack, coreference, compute_default13;
var init_compute13 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/coreference/compute/index.js"() {
    init_findPerson();
    init_findThey();
    init_lib12();
    addReference = function(pron, m3) {
      if (m3 && m3.found) {
        const term = pron.docs[0][0];
        term.reference = m3.ptrs[0];
      }
    };
    stepBack = function(m3, cb) {
      let s3 = m3.before();
      let res = cb(s3);
      if (res.found) {
        return res;
    ...
    compute_default13 = coreference;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/coreference/api/pronouns.js
var api20, pronouns_default;
var init_pronouns = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/coreference/api/pronouns.js"() {
    api20 = function(View2) {
    ...
    pronouns_default = api20;
  }
});

// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/coreference/plugin.js
var plugin_default30;
var init_plugin30 = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/3-three/coreference/plugin.js"() {
    init_compute13();
    init_pronouns();
    plugin_default30 = {
...
// node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/three.js
var three_default;
var init_three = __esm({
  "node_modules/.pnpm/compromise@14.15.0/node_modules/compromise/src/three.js"() {
    ...
    three_default = two_default;
  }
});

// packages/engine/src/extraction/heuristic.ts
...
function extractRegex(text) {
  ...
  for (const match2 of text.matchAll(REGEX.email)) {
    const email = match2[0];
    addFact(`User's email is ${email}`, email, ["contact", "email"]);
  }
  for (const match2 of text.matchAll(REGEX.phone)) {
    const phone = match2[0];
    addFact(`User's phone number is ${phone}`, phone, ["contact", "phone"]);
  }
  for (const match2 of text.matchAll(REGEX.url)) {
    const url = match2[0];
    addFact(`User mentioned URL: ${url}`, url, ["url"]);
  }
  for (const match2 of text.matchAll(REGEX.date)) {
    const date = match2[0];
    addFact(`Mentioned date: ${date}`, date, ["date"]);
  }
  for (const match2 of text.matchAll(REGEX.money)) {
    const amount = match2[0];
...
function extractPatterns(lines) {
  ...
  for (const rawLine of lines) {
    ...
    for (const rule of PATTERN_RULES) {
      const match2 = line.match(rule.pattern);
      if (match2) {
        const content = rule.template(match2);
        if (!seenContents.has(content)) {
          seenContents.add(content);
...
function extractEntities(text) {
  ...
  const places2 = doc.places().out("array");
  for (const place2 of places2) {
    addEntity(place2, "location");
  }
  return entities;
}
function extractHeuristic(text) {
  ...
  const regexContents = new Set(regexFacts.map((f3) => f3.content));
  const patternFacts = extractPatterns(lines);
  const filteredPatternFacts = patternFacts.filter((f3) => !regexContents.has(f3.content));
  const entities = extractEntities(text);
  const facts = [...regexFacts, ...filteredPatternFacts];
  const confidence = facts.length > 0 ? Math.max(...facts.map((f3) => f3.confidence)) : entities.length > 0 ? 0.6 : 0.5;
  return {
    facts,
    entities,
    edges: [],
  ...
  "packages/engine/src/extraction/heuristic.ts"() {
    ...
    PATTERN_RULES = [
      // Health / allergy — importance 0.95
      {
        ...
        template: (m3) => `User is allergic to ${m3[1].trim()}`,
        importance: 0.95,
        tags: ["health", "allergy"]
      },
      {
      ...
      },
      {
        ...
        template: (m3) => `User has ${m3[1].trim()}`,
        importance: 0.95,
        tags: ["health"]
      },
      // Name — importance 0.9
      {
        ...
        template: (m3) => `User's name is ${m3[1].trim()}`,
        importance: 0.9,
        tags: ["identity", "name"]
      },
      {
      ...
      // Identity — importance 0.85
      {
        ...
        template: (m3) => `User is a ${m3[1].trim()}`,
        importance: 0.85,
        tags: ["identity"]
      },
      {
        pattern: /\bi(?:'m| am)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)(?:[.!?,]|$)/,
        template: (m3) => `User is ${m3[1].trim()}`,
        importance: 0.85,
        tags: ["identity"]
      },
      // Work / company — importance 0.8
      {
        ...
        template: (m3) => `User works at ${m3[1].trim()}`,
        importance: 0.8,
        tags: ["work", "company"]
      },
      {
        pattern: /\bmy\s+(?:job|career|profession|occupation)\s+is\s+(.+?)(?:[.!?]|$)/i,
        template: (m3) => `User's job is ${m3[1].trim()}`,
        importance: 0.8,
        tags: ["work"]
      },
      {
        pattern: /\bi\s+(?:work|am employed)\s+as\s+(?:a(?:n)?\s+)?(.+?)(?:[.!?]|$)/i,
        template: (m3) => `User works as ${m3[1].trim()}`,
        importance: 0.8,
        tags: ["work"]
      },
      // Location — importance 0.7
      {
        ...
        template: (m3) => `User lives in ${m3[1].trim()}`,
        importance: 0.7,
        tags: ["location"]
      },
      {
        pattern: /\bi(?:'m| am)\s+from\s+(.+?)(?:[.!?]|$)/i,
        template: (m3) => `User is from ${m3[1].trim()}`,
        importance: 0.7,
        tags: ["location"]
      },
      {
        pattern: /\bi\s+(?:moved|relocated)\s+to\s+(.+?)(?:[.!?]|$)/i,
        template: (m3) => `User moved to ${m3[1].trim()}`,
        importance: 0.7,
        tags: ["location"]
      },
      // Preferences (like/love/enjoy) — importance 0.6
      {
        ...
        template: (m3) => `User likes ${m3[1].trim()}`,
        importance: 0.6,
        tags: ["preference", "like"]
      },
      {
        pattern: /\bmy\s+favorite\s+(?:\w+\s+)?is\s+(.+?)(?:[.!?]|$)/i,
        template: (m3) => `User's favorite is ${m3[1].trim()}`,
        importance: 0.6,
        tags: ["preference", "like"]
      },
      // Dislikes — importance 0.6
      {
        ...
        template: (m3) => `User dislikes ${m3[1].trim()}`,
        importance: 0.6,
      ...
      // Trivia / other — importance 0.3
      {
        ...
        template: (m3) => `User thinks ${m3[1].trim()}`,
        importance: 0.3,
        tags: ["opinion"]
      }
    ];
...
// packages/engine/src/extraction/prompts.ts
...
function buildGraphExtractionPrompt(facts, entityTypes) {
  const factsList = facts.map((f3, i3) => `${i3 + 1}. ${f3}`).join("\n");
...
var FACT_EXTRACTION_PROMPT, GRAPH_EXTRACTION_PROMPT, DEFAULT_ENTITY_TYPES;
var init_prompts = __esm({
  "packages/engine/src/extraction/prompts.ts"() {
    "use strict";
    FACT_EXTRACTION_PROMPT = `You are a memory extraction engine. Extract facts from text for a personal AI memory system.
...
Nothing else. No explanation, no markdown.`;
    GRAPH_EXTRACTION_PROMPT = `You are a knowledge graph builder. Given a list of facts about a person, extract entities and relationships.
...
Return ONLY valid JSON.`;
    DEFAULT_ENTITY_TYPES = ["person", "organization", "location", "technology", "concept", "event"];
  }
});

// packages/engine/src/extraction/sliding-window.ts
...
function createEnrichedSegments(text, config) {
  const c2 = { ...DEFAULT_CONFIG, ...config };
  if (text.length < c2.minInputLength) {
    return [{
      segment: text,
      contextWindow: text,
      index: 0,
  ...
  const segments = splitIntoSegments(text, c2.segmentSize);
  const effectiveSegments = segments.length > c2.maxSegments ? segments.slice(0, c2.maxSegments) : segments;
  return effectiveSegments.map((seg, i3) => {
    const prevStart = Math.max(0, i3 - c2.hPrev);
    const nextEnd = Math.min(segments.length - 1, i3 + c2.hNext);
    const contextBefore = segments.slice(prevStart, i3).join("\n");
    const contextAfter = segments.slice(i3 + 1, nextEnd + 1).join("\n");
    let contextWindow = "";
    if (contextBefore) {
...
var init_sliding_window = __esm({
  "packages/engine/src/extraction/sliding-window.ts"() {
    ...
    DEFAULT_CONFIG = {
      ...
      hNext: 1,
      minInputLength: 3500,
      // Window multi-turn conversations for pronoun/temporal resolution
      maxSegments: 6
      // Allow more segments for better coverage
    };
  }
});

// packages/engine/src/extraction/llm-extractor.ts
async function extractWithLLM(config, input, existingFacts) {
  let totalTokensIn = 0;
  ...
  for (let i3 = 0; i3 < segments.length; i3 += 4) {
    segmentBatches.push(segments.slice(i3, i3 + 4));
  }
  for (const batch of segmentBatches) {
    const batchPromises = batch.map(async (seg) => {
      ...
      if (existingFacts && existingFacts.length > 0) {
        const factsBlock = existingFacts.map((f3) => `- [lineage: ${f3.lineageId}] ${f3.content}`).join("\n");
...
${factsBlock}`;
      ...
      if (factEntries.length > 0) {
        const alreadyExtracted = factEntries.map((f3) => `- ${f3.text}`).join("\n");
        factMessages[1].content += `
...
${alreadyExtracted}`;
      ...
      try {
        ...
        for (const f3 of rawFacts) {
          if (typeof f3 === "string") {
            ...
            const eventDate = obj.ed ? new Date(obj.ed) : void 0;
            const documentDate = obj.dd ? new Date(obj.dd) : void 0;
            if (text.length > 0) entries.push({
              text,
              importance: Math.max(0, Math.min(1, importance)),
              sourceChunk: seg.contextWindow,
              eventDate: eventDate && !isNaN(eventDate.getTime()) ? eventDate : void 0,
              documentDate: documentDate && !isNaN(documentDate.getTime()) ? documentDate : void 0
            });
          }
        }
        return entries;
  ...
  factEntries = factEntries.filter((e2) => {
    const key = e2.text.toLowerCase().trim();
    if (seenContent.has(key)) return false;
    seenContent.add(key);
    return true;
  });
  factStrings = factEntries.map((e2) => e2.text);
  if (factEntries.length === 0) {
  ...
  for (const fact of facts) {
    ...
    const contextPrefix = src.length > 100 ? `Context: ${src.slice(0, 200).trim()}... | Fact: ` : `Context: ${src.trim()} | Fact: `;
  ...
  try {
    ...
    if (Array.isArray(graphParsed.entities)) {
      for (const e2 of graphParsed.entities) {
        if (!e2 || typeof e2.name !== "string") continue;
        const entity2 = e2;
        const canonical = normalizeEntityName(entity2.name);
        ...
        entities.push({
          ...
          entityType: String(entity2.entity_type ?? entity2.type ?? "concept"),
          canonicalName: canonical,
          properties: {}
        });
      }
    ...
    if (Array.isArray(graphParsed.edges)) {
      for (const r2 of graphParsed.edges) {
        if (!r2) continue;
        const rel = r2;
        const rawSource = typeof rel.source === "string" ? rel.source : typeof rel.source_name === "string" ? rel.source_name : null;
        const rawTarget = typeof rel.target === "string" ? rel.target : typeof rel.target_name === "string" ? rel.target_name : null;
    ...
    for (const fact of facts) {
      ...
      for (const entity2 of entities) {
        if (entity2.canonicalName === "user") {
          if (contentLower.startsWith("user ") || contentLower.includes(" user ")) {
            fact.entityCanonicalNames.push(entity2.canonicalName);
          }
        } else if (entity2.canonicalName.length >= 3 && contentLower.includes(entity2.canonicalName)) {
          fact.entityCanonicalNames.push(entity2.canonicalName);
        }
      }
    }
...
function emptyResult(tier, model5) {
  return {
    facts: [],
    entities: [],
    edges: [],
...
function isValidEdgeType(t3) {
  return typeof t3 === "string" && ["associative", "causal", "temporal", "contradictory", "hierarchical"].includes(t3);
}
...
// packages/engine/src/extraction/dedup.ts
async function deduplicateFacts(config, facts, tenantId, scope, scopeId) {
  ...
  for (const fact of facts) {
    ...
    const matches3 = await config.storage.vectorSearch({
      embedding,
      tenantId,
      scope,
      scopeId,
    ...
    const decision = await classifyWithLLM(
      ...
      matches3.map((m3) => ({
        id: m3.fact.id,
        lineageId: m3.fact.lineageId ?? "",
        content: m3.fact.content,
        similarity: m3.similarity
      }))
    );
...
EXISTING FACTS:
${existingMatches.map((m3) => `[id: ${m3.id}, lineage_id: ${m3.lineageId}, similarity: ${m3.similarity.toFixed(3)}] ${m3.content}`).join("\n")}
...
Classify the new fact.`
  ...
  try {
    ...
    const relationType = operation === "update" ? "updates" : operation === "contradict" ? "updates" : void 0;
    const relatedFactId = (operation === "update" || operation === "contradict") && existingMatches.length > 0 ? existingMatches[0].id : void 0;
    return {
      operation,
      existingLineageId: typeof parsed.existing_lineage_id === "string" ? parsed.existing_lineage_id : void 0,
      contradictsFactId: typeof parsed.contradicts_fact_id === "string" ? parsed.contradicts_fact_id : void 0,
      relationType,
      relatedFactId
    };
  } catch {
...
// packages/engine/src/extraction/contradiction.ts
function processContradictions(facts) {
...
// packages/engine/src/extraction/entity-extractor.ts
async function buildEntityIdMap(storage, embedding, tenantId, entities) {
  const entityIdMap = /* @__PURE__ */ new Map();
  let entitiesCreated = 0;
  const uniqueEntities = /* @__PURE__ */ new Map();
  for (const entity2 of entities) {
    if (!uniqueEntities.has(entity2.canonicalName)) {
      uniqueEntities.set(entity2.canonicalName, entity2);
    }
  }
  for (const entity2 of uniqueEntities.values()) {
    ...
    if (existing) {
      entityIdMap.set(entity2.canonicalName, existing.id);
    } else {
      const id = crypto.randomUUID();
      const emb = await embedding.embed(entity2.name);
      ...
      entityIdMap.set(entity2.canonicalName, id);
      entitiesCreated++;
    }
  }
...
async function persistEdges(storage, tenantId, factId, edges, entityIdMap) {
  ...
  for (const edge of dedupedEdges) {
    ...
    if (!sourceId || !targetId) {
      console.warn(
        `[steno] Edge dropped: "${edge.sourceName}" \u2192 "${edge.relation}" \u2192 "${edge.targetName}" (source=${sourceId ? "found" : "MISSING"}, target=${targetId ? "found" : "MISSING"}) entityMap keys: [${[...entityIdMap.keys()].join(", ")}]`
      );
    }
  }
  return edgesCreated;
...
// packages/engine/src/extraction/hasher.ts
...
// packages/engine/src/scratchpad/scratchpad.ts
async function getScratchpad(storage, tenantId, scope, scopeId) {
  const facts = await storage.getFactsByScope(tenantId, scope, scopeId, { limit: 100 });
  const scratchpadFact = facts.data.find((f3) => f3.tags?.includes("scratchpad"));
  return scratchpadFact?.content ?? "";
}
async function updateScratchpad(storage, llm, tenantId, scope, scopeId, newFacts) {
...
${newSection}` : newSection;
  ...
  const existingScratchpad = facts.data.find((f3) => f3.tags?.includes("scratchpad"));
  ...
  await storage.createFact({
    ...
    modality: "text",
    tags: ["scratchpad"],
    metadata: { type: "scratchpad", updatedAt: (/* @__PURE__ */ new Date()).toISOString() },
    contradictionStatus: "none"
  });
}
async function compressScratchpad(llm, content) {
...
var SCRATCHPAD_MAX_CHARS, SCRATCHPAD_COMPRESSED_CHARS;
var init_scratchpad = __esm({
  "packages/engine/src/scratchpad/scratchpad.ts"() {
    "use strict";
    SCRATCHPAD_MAX_CHARS = 5e3;
    SCRATCHPAD_COMPRESSED_CHARS = 2500;
  }
});

// packages/engine/src/extraction/pipeline.ts
var pipeline_exports = {};
__export(pipeline_exports, {
  inputToText: () => inputToText,
  mergeEntities: () => mergeEntities,
  mergeFacts: () => mergeFacts,
  runExtractionFromQueue: () => runExtractionFromQueue,
  runExtractionPipeline: () => runExtractionPipeline
});
  ...
  if (typeof input.data === "object" && input.data !== null) {
    ...
    if (Array.isArray(data.messages)) {
      return data.messages.filter(
        (m3) => typeof m3 === "object" && m3 !== null && typeof m3.role === "string" && typeof m3.content === "string"
      ).map((m3) => `${m3.role}: ${m3.content}`).join("\n");
    }
    return JSON.stringify(input.data);
  }
  return String(input.data);
}
function mergeFacts(heuristic, llm) {
  const llmContents = new Set(llm.map((f3) => f3.content.toLowerCase()));
  const unique2 = heuristic.filter((f3) => !llmContents.has(f3.content.toLowerCase()));
  return [...llm, ...unique2];
}
function mergeEntities(heuristic, llm) {
  const seen = /* @__PURE__ */ new Map();
  for (const e2 of [...heuristic, ...llm]) {
    const normalized = normalizeEntityName(e2.canonicalName);
    if (normalized.length === 0) continue;
    seen.set(normalized, { ...e2, canonicalName: normalized });
  }
...
async function executeExtraction(config, extractionId, input, startTime) {
  ...
  if (tier !== "heuristic_only") {
    ...
    let existingFactsForLLM = input.existingFacts?.map((f3) => ({
      lineageId: f3.lineageId,
      content: f3.content
    }));
    if (!existingFactsForLLM || existingFactsForLLM.length === 0) {
      try {
        const queryEmbedding = await config.embedding.embed(textContent.slice(0, 1e3));
        ...
        if (similar.length > 0) {
          existingFactsForLLM = similar.map((s3) => ({
            lineageId: s3.fact.lineageId ?? s3.fact.id,
            content: s3.fact.content
          }));
        }
      } catch {
      }
    ...
    if (tier === "auto" && llmResult.confidence < 0.6 && config.smartLLM) {
  ...
  const tierUsed = uniqueTiers.length > 1 ? "multi_tier" : uniqueTiers[0] === "heuristic" ? "heuristic" : uniqueTiers[0] === "cheap_llm" ? "cheap_llm" : "smart_llm";
  if (!mergedEntities.some((e2) => e2.canonicalName === "user")) {
    mergedEntities.push({
      name: "User",
      entityType: "person",
      canonicalName: "user",
  ...
  let firstFactId;
  const factsToEmbed = contradictionResults.filter((r2) => r2.fact.operation !== "noop");
  const factTexts = factsToEmbed.map((r2) => r2.fact.contextualContent ?? r2.fact.content);
  ...
  for (const { fact, contradictionStatus, contradictsId } of contradictionResults) {
    ...
    await config.storage.createFact({
      ...
      operation: fact.operation === "add" || fact.operation === void 0 ? "create" : fact.operation === "update" ? "update" : fact.operation === "invalidate" ? "invalidate" : "create",
      contradictionStatus,
      contradictsId: contradictsId ?? void 0,
  ...
  if (mergedEdges.length > 0) {
    for (const e2 of mergedEdges.slice(0, 5)) {
      console.error(`[steno]   edge: "${e2.sourceName}" \u2192 "${e2.relation}" \u2192 "${e2.targetName}"`);
    }
  }
  if (firstFactId !== void 0 && mergedEdges.length > 0) {
  ...
  try {
    const factContents = contradictionResults.filter((r2) => r2.fact.operation !== "noop").map((r2) => r2.fact.content);
...
async function runExtractionPipeline(config, input) {
  const startTime = Date.now();
...
async function runExtractionFromQueue(config, extractionId, input) {
  const startTime = Date.now();
  await config.storage.updateExtraction(input.tenantId, extractionId, {
    status: "processing"
  });
...
// packages/engine/src/sessions/manager.ts
var manager_exports = {};
...
async function getOrCreateActiveSession(storage, tenantId, scope, scopeId) {
  const sessions = await storage.getSessionsByScope(tenantId, scope, scopeId, { limit: 5 });
  const active = sessions.data.find((s3) => !s3.endedAt);
  if (active) return active;
  return startSession(storage, tenantId, scope, scopeId);
}
async function startSession(storage, tenantId, scope, scopeId, metadata) {
  const id = crypto.randomUUID();
...
async function endSession(storage, llm, tenantId, sessionId) {
  ...
  const sessionFacts = facts.data.filter((f3) => f3.sessionId === sessionId);
  let summary;
  let topics;
  if (sessionFacts.length > 0) {
    const factContents = sessionFacts.map((f3) => f3.content).join("\n");
    const response = await llm.complete([
      {
        role: "system",
    ...
    try {
      ...
      topics = Array.isArray(parsed.topics) ? parsed.topics.filter((t3) => typeof t3 === "string") : void 0;
    } catch {
...
// packages/engine/src/adapters/perplexity-embedding.ts
...
// packages/mcp-server/src/local.ts
import { StdioServerTransport } from "@modelcontextprotocol/sdk/server/stdio.js";

// packages/mcp-server/src/local-server.ts
import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import { z } from "zod";
var FLUSH_DELAY_MS = 3e4;
...
4. Steno memory persists across ALL conversations \u2014 it knows things from past sessions that your conversation history does not.`
  ...
  async function getPipeline() {
    if (!_pipeline) {
      const mod = await Promise.resolve().then(() => (init_pipeline(), pipeline_exports));
  ...
  async function getSessionManager() {
    if (!_getOrCreateActiveSession) {
      const mod = await Promise.resolve().then(() => (init_manager(), manager_exports));
  ...
  server.tool(
    ...
    async (args) => {
      ...
      buf.lastActivity = /* @__PURE__ */ new Date();
      ...
      if (isSingleFact && buf.messages.length === 1) {
        ...
        if (matches3.length > 0) {
          ...
          buf.messages = [];
          return {
            content: [{ type: "text", text: `Remembered (1 fact updated, 0 entities, 0 edges)` }]
          };
        }
        await config.storage.createFact({
          id: factId,
        ...
        buf.messages = [];
        return {
          content: [{ type: "text", text: `Remembered (1 fact, 0 entities, 0 edges)` }]
        };
      }
      scheduleFlush(key);
  ...
  server.tool(
    ...
    async ({ query, limit }) => {
      ...
      const text = results.results.map((r2) => {
        const dateParts = [];
        if (r2.fact.eventDate) dateParts.push(`event: ${new Date(r2.fact.eventDate).toISOString().slice(0, 10)}`);
        if (r2.fact.documentDate) dateParts.push(`doc: ${new Date(r2.fact.documentDate).toISOString().slice(0, 10)}`);
        const dateStr = dateParts.length > 0 ? `, ${dateParts.join(", ")}` : "";
        const signals = Object.entries(r2.signals).filter(([, v2]) => v2 > 0).map(([k2, v2]) => `${k2.replace("Score", "")}=${v2.toFixed(2)}`).join(", ");
        let line = `[Memory] ${r2.fact.content} (score: ${r2.score.toFixed(2)}${dateStr}${signals ? `, ${signals}` : ""})`;
        if (r2.fact.sourceChunk) {
          line += `
[Source Context] ${r2.fact.sourceChunk}`;
        }
        line += "\n---";
        return line;
      }).join("\n");
...
async function main() {
  ...
  const { createSupabaseClient: createSupabaseClient2, SupabaseStorageAdapter: SupabaseStorageAdapter2 } = await Promise.resolve().then(() => (init_src3(), src_exports));
  const { OpenAILLMAdapter: OpenAILLMAdapter2 } = await Promise.resolve().then(() => (init_src4(), src_exports2));
  ...
  if (process.env.PERPLEXITY_API_KEY) {
    ...
    embedding = new OpenAIEmbeddingAdapter2({
      apiKey: openaiKey,
