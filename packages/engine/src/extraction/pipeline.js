...
async function executeExtraction(config, extractionId, input, startTime) {
    ...
    if (tier !== 'heuristic_only') {
        ...
        const llmResult = await extractWithLLM({ llm: llmToUse, tier: llmTier }, textContent, existingFactsForLLM);
        ...
        if (tier === 'auto' &&
            ...
            config.smartLLM) {
            const smartResult = await extractWithLLM({ llm: config.smartLLM, tier: 'smart_llm' }, textContent, existingFactsForLLM);
